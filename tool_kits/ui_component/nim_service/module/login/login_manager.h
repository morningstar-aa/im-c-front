...
public:
	...
	*/
	...
	void SetNickName(const std::string &account);
	void SetLoginName(const std::string &account);
	...
	* @return void	无返回值
	*/
	void SetClientSetting(const std::map<std::string, std::string> &map_);

	/**
	* 获取客户端设置
	* @return string 用户id
	*/
	std::map<std::string, std::string> & GetClientSetting();
	...
	* 设置用户token
	*/
	void SetAccountToken(const std::string& token);
	/**
	* 获取用户token
	*/
...
private:
	...
	std::map<std::string, std::string> client_setting_map_;
	LOG_LEVEL demo_log_level_ = LV_APP;
	int limit_file_size_ = 300;
	bool team_msg_ack_;
	std::string nick_;
	std::string login_;
