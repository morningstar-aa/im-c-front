...
{
	//cjy2 过滤掉红包消息
	//自定义消息 message.attach_.find("\"type\":100") != std::string::npos ||
	if ((message.type_ == nim::kNIMMessageTypeCustom && (message.attach_.find("\"type\":101") != std::string::npos)) || (message.type_ == nim::kNIMMessageTypeTips && message.content_ == "")){
		return;
	}
	...
	if (message.feature_ == nim::kNIMMessageFeatureDefault)
	{
		...
		else
		{
			...
			std::vector<int> aniIndex{ 1 };
			TrayIconManager::GetInstance()->SetAnimateTray(aniIndex, 1000);
			TrayIconManager::GetInstance()->StartTrayIconAnimate();
		}
	}
