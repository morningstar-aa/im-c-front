...
#include "tool_kits/ui_component/ui_kit/export/nim_ui_contacts_list_manager.h"
...
/*void TeamCallback::UITeamCreateCallback(int res, const std::string& err_msg)
{
	if (res == 0)
	{
	}
...
void TeamCallback::OnTeamEventCallbackEx(const int type, const nim::TeamInfo tInfo, const nim::TeamEvent& result)
{
	nim::Team::QueryTeamInfoAsync(result.team_id_, nbase::Bind(&TeamCallback::UITeamEventCallback, result, std::placeholders::_1, std::placeholders::_2));
	QLOG_APP(L"OnTeamEventCallbackEx: notify_id={0} code={1} tid={2}") << result.notification_id_ << result.res_code_ << result.team_id_;

	nim::TeamEvent tresult = result;
	std::string tName = tInfo.GetName();
	tresult.team_info_.SetName(tName);
	tresult.team_info_.SetMemberAddFriendMode(tInfo.GetMemberAddFriendMode());
	if (type == 0)
	{
		nim_ui::ContactsListManager::GetInstance()->SaveCreateTeamInfo(tresult);
	}
	else if (type == 1)
	{
		nim_ui::ContactsListManager::GetInstance()->UpdateTeamInfo(tresult);
	}
	else if (type == 2)
	{
		nim_ui::ContactsListManager::GetInstance()->DissmissTeam(tresult);

		nim_ui::SessionListManager::GetInstance()->DeleteLatestContact(tInfo.GetTeamID());
		//nim::Session::DeleteRecentSession(nim::kNIMSessionTypeTeam, tInfo.GetTeamID(), nbase::Bind(&SessionItem::DeleteRecentSessionCb, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
	}
	else if (type == 3)
	{
		nim::Session::DeleteRecentSession(nim::kNIMSessionTypeTeam, tInfo.GetTeamID(), nbase::Bind(&SessionItem::DeleteRecentSessionCb, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
...
bool ParsCustommsg(nim::SysMessage& msg)
{
	/*if (msg.sender_accid_ != "")
	{
	return false;
	}*/
	Json::Value json;
	if (StringToJson(msg.attach_, json) && json.isObject())
	{
		if (json.isMember("type"))
		{
			...
			if (type == 1)
			{
				...
				if (!jsonValue["islogin"].isNull())
				{
					islogin = jsonValue["islogin"].asString();
				}

				if (islogin == "0")
				{
					QCommand::Set(kCmdRestart, L"true");
					std::wstring wacc = nbase::UTF8ToUTF16(nim_comp::LoginManager::GetInstance()->GetAccount());
...
void TeamCallback::QueryTeamInfoCallback(const std::string& tid, const nim::TeamInfo& result)
{
	SessionBox* session = SessionManager::GetInstance()->FindSessionBox(tid);
	if (session)
	{
		bool tag = result.GetMuteType();
		session->setSessionMuteTag(tag);
	}
}
}