...
const std::vector<std::wstring> TeamInfoBox::options_name_list_ = {
	...
	L"up_addfriend_mode_allow",         L"up_addfriend_mode_notallow",
...
TeamInfoBox::TeamInfoBox(bool create_or_display, nim::NIMTeamType type, const std::string& team_id, const nim::TeamInfo& team_info, bool value)
{
	...
	{
		//从服务器获取能否添加好友
		nim_ui::UserManager::GetInstance()->InvokeGetMemberAddFriend(tid_, ToWeakCallback([this](int code, const std::string &msg, const std::string &canAddFriend) {
			if (code == 0)
			{
				if (canAddFriend == "1")
				{
					((ui::Option*)FindSubControl(L"up_addfriend_mode_allow"))->Selected(true);
				}
				else
				{
					((ui::Option*)FindSubControl(L"up_addfriend_mode_notallow"))->Selected(true);
				}
			}
			else
			{
...
void TeamInfoBox::DoInit()
{
	...
	for (auto it : options_name_list_)
	{
		...
		if (option != nullptr)
		{
			std::wstring str = option->GetGroup().append(nbase::UTF8ToUTF16(team_info_.GetTeamID())).append(view_mode_ ? L"true" : L"false");
...
ui::HBox* TeamInfoBox::CreateTeamMemberListItem(const nim::TeamMemberProperty& member_info, bool delelte_enable/* = false*/)
{
	...
	if (!create_or_display_ && team_info_.GetType() == nim::kNIMTeamTypeAdvanced && has_authority)//(is_me || has_authority))
...
void TeamInfoBox::OnMsgboxQuitTeam(MsgBoxRet ret)
{
	if (ret == MB_YES)
	{
		nim::TeamInfo tinfo;
		tinfo.SetTeamID(tid_);
		nim::Team::LeaveAsync(tid_, nbase::Bind(&TeamCallback::OnTeamEventCallbackEx, 3, tinfo, std::placeholders::_1));
		//nim::Team::DismissAsync(tid_, std::bind(&TeamCallback::OnTeamEventCallbackEx, 3, nim::TeamInfo(), std::placeholders::_1));
		Close();
	}
}

void TeamInfoBox::OnMsgboxDissolveTeam(MsgBoxRet ret)
{
	if (ret == MB_YES)
	{
		nim::TeamInfo tinfo;
		tinfo.SetTeamID(tid_);
		nim::Team::DismissAsync(tid_, std::bind(&TeamCallback::OnTeamEventCallbackEx, 2, tinfo, std::placeholders::_1));
		Close();
	}
}
...
bool TeamInfoBox::OnBtnConfirmClick(ui::EventArgs* param)
{
	...
	if (type_ == nim::kNIMTeamTypeAdvanced)
	{
		...
		nim::NIMTeamMemberAddFriendMode up_addfriend_mode = nim::kNIMTeamMemberAddFriendModeNotAllow;
		bool a = ((ui::Option*)FindSubControl(L"up_addfriend_mode_notallow"))->IsSelected();
		if (((ui::Option*)FindSubControl(L"up_addfriend_mode_notallow"))->IsSelected())
			up_addfriend_mode = nim::kNIMTeamMemberAddFriendModeNotAllow;
		else if (((ui::Option*)FindSubControl(L"up_addfriend_mode_allow"))->IsSelected())
			up_addfriend_mode = nim::kNIMTeamMemberAddFriendModeAllow;
		
		if (up_addfriend_mode != team_info_.GetMemberAddFriendMode())
			tinfo.SetMemberAddFriendMode(up_addfriend_mode); 
	}
	else {
	...
	if (create_or_display_)
	{
		...
		//tinfo.SetMemberMaxCount(1000);
		if (!tinfo.GetIcon().empty())
			PhotoService::GetInstance()->DownloadTeamIcon(tinfo);
		nim::Team::CreateTeamAsync(tinfo, id_list, "", std::bind(&TeamCallback::OnTeamEventCallbackEx, 0, tinfo, std::placeholders::_1));
	}
	else
    {
        // 群消息通知方式
        ...
        if (FindSubControl(L"team_owner_section")->IsEnabled() || FindSubControl(L"team_name_panel")->IsEnabled())
        {
            ...
            // 更新群组基本信息
			nim::Team::UpdateTeamInfoAsync(tid_, tinfo, std::bind(&TeamCallback::OnTeamEventCallbackEx, 1, tinfo, std::placeholders::_1));
		}
	}
...
{
	auto cb = nbase::Bind(&TeamInfoBox::OnMsgboxDissolveTeam, this, std::placeholders::_1);
	ShowMsgBox(NULL, cb, L"STRID_TEAM_INFO_DISSOLVE_TEAM_TIP", true, L"STRING_TIPS", true, L"STRING_OK", true, L"STRING_NO", true);
	//nim::Team::DismissAsync(tid_, std::bind(&TeamCallback::OnTeamEventCallback, std::placeholders::_1));
	/*nim::Team::DismissAsync(tid_, std::bind(&TeamCallback::OnTeamEventCallbackEx, 2, "", std::placeholders::_1));
	Close();*/
	return true;
}
...
bool TeamInfoBox::OnBtnQuitClick(ui::EventArgs* param)
{
	...
	Close();*/
	auto cb = nbase::Bind(&TeamInfoBox::OnMsgboxQuitTeam, this, std::placeholders::_1);
	ShowMsgBox(NULL, cb, L"STRID_TEAM_INFO_QUIT_TEAM_TIP", true, L"STRING_TIPS", true, L"STRING_OK", true, L"STRING_NO", true);
	return true;
}
