...
namespace nim_comp
{
void ContactSelectSendForm::AddGroup(ui::TreeNode* tree_node)
{
	...
	ContactTileListSendUI* tile_layout = new ContactTileListSendUI;
...
void ContactSelectSendForm::AddFriendListItem(const std::string& accid, bool is_enable)
{
	...
	ContactTileListSendUI* tile_layout;
	if (!ws_spell.empty())
	{
...
void ContactSelectSendForm::RemoveFriendListItem(const std::string& accid)
{
	wstring ws_show_name = UserService::GetInstance()->GetUserName(accid);
	string spell = PinYinHelper::GetInstance()->ConvertToFullSpell(ws_show_name);
	wstring ws_spell = nbase::UTF8ToUTF16(spell);
...
void ContactSelectSendForm::AddTeamListItem(const std::string& teamid, bool is_enable)
{
	ContactTileListSendUI* tile_layout = GetGroup(GT_TEAM);
...
void ContactSelectSendForm::RemoveTeamListItem(const std::string& teamid)
{
	ContactTileListSendUI* tile_layout = GetGroup(GT_TEAM);
...
ui::Box* ContactSelectSendForm::AddListItemInGroup(const std::string& accid, bool is_team, ContactTileListSendUI* tile_layout)
{
	...
	ContactListItemSendUI* container_element = CreateListItem(accid, is_team);
	int index = 0;
	for (index = 0; index < tile_layout->GetCount(); index++)
	{
		ContactListItemSendUI* temp = (ContactListItemSendUI*)tile_layout->GetItemAt(index);
...
bool ContactSelectSendForm::RemoveListItemInGroup(const std::string& accid, ContactTileListSendUI* tile_layout)
{
	bool ret = false;
	int index = 0;
	for (index = 0; index < tile_layout->GetCount(); index++)
...
ContactListItemSendUI* ContactSelectSendForm::CreateListItem(const std::string& accid, bool is_team)
{
	ContactListItemSendUI* container_element = new ContactListItemSendUI();
...
SelContactItemSendUI* ContactSelectSendForm::CreateSelectedListItem(const std::string& accid, bool is_team)
{
	SelContactItemSendUI* selected_item = new SelContactItemSendUI();
	selected_item->Init(accid, is_team);

	Button* btn_delete = (Button*)selected_item->FindSubControl(L"delete");
	btn_delete->AttachClick(nbase::Bind(&ContactSelectSendForm::OnBtnDeleteClick, this, accid, std::placeholders::_1));

	return selected_item;
}

ContactTileListSendUI* ContactSelectSendForm::GetGroup(GroupType groupType, wchar_t letter)
{
	return tree_node_ver_[ListItemUtil::GetGroup(groupType, letter, false)];
}

void ContactSelectSendForm::OnFriendListChange(FriendChangeType change_type, const std::string& accid)
{
...
void ContactSelectSendForm::OnSetBlackCallback(const std::string& id, bool black)
{
	UTF8String current_user_id = LoginManager::GetInstance()->GetAccount();
...
void ContactSelectSendForm::OnUserInfoChange(const std::list<nim::UserNameCard>& uinfos)
{
	...
	for (auto info : uinfos)
	{
		...
		{
			//好友列表项
			ContactListItemSendUI* friend_item = dynamic_cast<ContactListItemSendUI*>(contract_list_->FindSubControl(nbase::UTF8ToUTF16(accid)));
			if(!friend_item)
				continue;

			//搜索列表项
			ContactListItemSendUI* search_item = NULL;
			if(search_result_list_->IsVisible())
				search_item = dynamic_cast<ContactListItemSendUI*>(search_result_list_->FindSubControl(nbase::UTF8ToUTF16(accid)));

			//选中列表项
			SelContactItemSendUI* select_item = dynamic_cast<SelContactItemSendUI*>(selected_user_list_->FindSubControl(nbase::UTF8ToUTF16(accid)));
			...
			if (info.ExistValue(nim::kUserNameCardKeyName))
			{
				...
				friend_item = dynamic_cast<ContactListItemSendUI*>(contract_list_->FindSubControl(nbase::UTF8ToUTF16(accid)));
...
void ContactSelectSendForm::OnUserPhotoReady(PhotoType type, const std::string & accid, const std::wstring & photo_path)
{
	if (type == kUser)
	{
	...
	if (search_result_list_->IsVisible())
	{
		ContactListItemSendUI* search_item = dynamic_cast<ContactListItemSendUI*>(search_result_list_->FindSubControl(nbase::UTF8ToUTF16(accid)));
		if (search_item)
			search_item->FindSubControl(L"head_image")->SetBkImage(photo_path);
	}
}

void ContactSelectSendForm::OnAddTeam(const std::string& teamid, const std::string& tname, nim::NIMTeamType type)
{
	OnRemoveTeam(teamid);
	AddTeamListItem(teamid, true);
}

void ContactSelectSendForm::OnRemoveTeam(const std::string& teamid)
{
	RemoveTeamListItem(teamid);
}

void ContactSelectSendForm::OnTeamNameChanged(const nim::TeamInfo& team_info)
{
	std::string accid = team_info.GetTeamID();

	//群组列表项
	ContactListItemSendUI* team_item = dynamic_cast<ContactListItemSendUI*>(contract_list_->FindSubControl(nbase::UTF8ToUTF16(accid)));
	if (!team_item)
		return;
	...
	if (team_info.ExistValue(nim::kNIMTeamInfoKeyName))
	{
		...
		team_item = dynamic_cast<ContactListItemSendUI*>(contract_list_->FindSubControl(nbase::UTF8ToUTF16(accid)));
