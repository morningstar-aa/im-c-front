...
void SessionPluginPage::FlashTaskbar(SessionBox &session_box)
{
	taskbar_manager_.SetTabActive(*session_box.GetTaskbarItem());

	FLASHWINFO flash_info = { sizeof(FLASHWINFO), this->GetHWND(), FLASHW_TRAY, 3, 0 };
	...
	if (flash || create)
	{
		...
		else
		{
			if (::IsIconic(hWnd))
				...
				need = true;
			else if (!(::GetForegroundWindow() == hWnd) || !IsWindowVisible(hWnd))
			{
				need = true;
			}
		}
