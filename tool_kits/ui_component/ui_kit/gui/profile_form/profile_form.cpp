...
ProfileForm *ProfileForm::ShowProfileForm(UTF8String tid, UTF8String uid, nim::NIMTeamUserType my_type)
{
	...
	else
	{
		...
		UserService::GetInstance()->GetUserInfo(uid, info);
		form->InitUserInfo(info);
		if (my_type == nim::kNIMTeamUserTypeCreator || my_type == nim::kNIMTeamUserTypeManager)
		{
			form->userAddFriend(true);
		}
		else
		{
			form->userAddFriend(false);
		}
	}
	if (!::IsWindowVisible(form->m_hWnd))
	{
...
void ProfileForm::userAddFriend(bool type)
{
	/*
	if (type)
	{
...
void ProfileForm::canAddFriend(std::string id)
{
	//std::string userId = LoginManager::GetInstance()->GetAccount();
	//nim_ui::UserManager::GetInstance()->InvokeCheckAddFriend(userId, id, ToWeakCallback([this](int code, const std::string& err_msg, const std::string addfriend) {
	//	if (code == 0)
	//	{
			/*if ("1" == addfriend)
			{
...
//void ProfileForm::IntUserId()
//{
//	UTF8String uid = m_uinfo.GetAccId();//search_key_->GetUTF8Text();
//	nbase::StringTrim(uid);
//	if (uid.empty())
//		return;
//
//	nim_ui::UserManager::GetInstance()->QueryCurMemberInfo(uid, ToWeakCallback([this](int code, const std::string msg,const UserBaseInfo userInfo) {
//		if (code == 0)
//		{ 
//			if (-1 == user_type)
//			{
//				m_uinfo.SetUserAccid(userInfo.account);
//				std::wstring account = nbase::StringPrintf(ui::MutiLanSupport::GetInstance()->GetStringViaID(L"STRID_PROFILE_FORM_ACCOUNT_").c_str(), nbase::UTF8ToUTF16(m_uinfo.GetUserAccid()).c_str());
//				user_id_label->SetText(account);//帐号
//			}
//			else
//			{
//				user_id_label->SetText(L"");
//			}
//		}
//		else
//		{
