...
namespace nim_comp
{
	...
	void ProfileBox::DoInit()
	{
		...
		show_name_label = static_cast<ui::RichEdit*>(FindSubControl(L"show_name"));
		user_id_label = static_cast<ui::RichEdit*>(FindSubControl(L"userid"));
		user_name_label = static_cast<ui::RichEdit*>(FindSubControl(L"username"));
		nickname_label = static_cast<ui::RichEdit*>(FindSubControl(L"nickname"));
		...
		//webrtc_setting_->SetVisible(false);
	...
	void ProfileBox::InitUserInfo(const nim::UserNameCard &info)
	{
		...
		if (user_type == -1) // 自己的名片
		{
			...
			email_edit->SetLimitText(100);
			signature_edit->SetLimitText(100);
			...
			user_id_label->AttachKillFocus(nbase::Bind(&ProfileBox::OnIdLoseFocus, this, std::placeholders::_1));
			user_name_label->AttachKillFocus(nbase::Bind(&ProfileBox::OnNameLoseFocus, this, std::placeholders::_1));
			show_name_label->AttachKillFocus(nbase::Bind(&ProfileBox::OnIdLoseFocus, this, std::placeholders::_1));
			nickname_label->AttachKillFocus(nbase::Bind(&ProfileBox::OnIdLoseFocus, this, std::placeholders::_1));
		}
		else
		{
		...
		std::string sphone = m_uinfo.GetMobile();
		if (sphone.empty()) {
			nim_ui::UserManager::GetInstance()->QueryMemberInfo(m_uinfo.GetAccId(), ToWeakCallback([this](int code, const std::string msg, const std::string uid, const std::string nickname, const std::string avatar, const std::string username) {
				if (code == 0)
				{
					user_name_label->SetText(L"用户名：" + nbase::UTF8ToUTF16(username));
					m_uinfo.SetMobile(username);
				}
				else
				{
			...
			user_name_label->SetText(L"用户名：" + nbase::UTF8ToUTF16(sphone));
		}
	}
	...
	void ProfileBox::OnDeleteFriendMsgBox(MsgBoxRet ret)
	{
		if (ret == MB_YES)
		{
			...
			Control* control = FindSubControl(L"chkbox_delete_with_alias");
			if (control)
			{
				option.delete_alias_ = dynamic_cast<ui::CheckBox*>(control)->IsSelected();
			}
			...
			std::string accId = m_uinfo.GetAccId();
			//nim_ui::UserManager::GetInstance()->InvokeDeleteFriend(userId, accId, ToWeakCallback([this](int code, const std::string &msg) {
			//	if (code == 0)
			//	{
			//	}
			//	else
	...
	{
		nim::Friend::Request(m_uinfo.GetAccId(), nim::kNIMVerifyTypeAsk, "", ToWeakCallback([this](int res_code) {
	...
	bool ProfileBox::OnIdLoseFocus(ui::EventArgs * args)
	{
		user_id_label->SetText(L"用户ID:"+ nbase::UTF8ToUTF16(m_uinfo.GetAccId()));
		SetShowName();

		return true;
	}

	bool ProfileBox::OnNameLoseFocus(ui::EventArgs * args)
	{
		user_name_label->SetText((L"用户名：" + nbase::UTF8ToUTF16(m_uinfo.GetMobile())));
