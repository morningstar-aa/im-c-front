...
void AddFriendWindow::InitWindow()
{
	...
	addfriend_msg_ = (RichEdit*)userinfo_page->FindSubControl(L"add_friend_msg");
	...
	add_success_page_ok->AttachClick(nbase::Bind(&AddFriendWindow::ChatClick, this, std::placeholders::_1));
...
bool AddFriendWindow::Search(ui::EventArgs* param)
{
	...
	nim_ui::UserManager::GetInstance()->QueryMemberInfoByAcc(key, ToWeakCallback([this](int code, const std::string msg, const std::string uid, const std::string nickname, const std::string avatar, const std::string username) {
		if (code == 0)
		{
			std::list<nim::UserNameCard> uinfos;
			nim::UserNameCard uinfo;
			uinfo.SetAccId(uid);
			uinfo.SetName(nickname);
			uinfo.SetIconUrl(avatar);
			//uinfo.SetUserAccid(search_key_->GetUTF8Text());
...
void AddFriendWindow::InitUserProfile(const nim::UserNameCard& uinfo)
{
	...
	userinfo_page_add_friend_->SetEnabled(!LoginManager::GetInstance()->IsEqual(id_));
	...
	std::string account = uinfo.GetAccId();
	if (!account.empty())
	{
		((Label*)FindControl(L"id"))->SetUTF8Text(account);
	}
	else
	{
		((Label*)FindControl(L"id"))->SetUTF8Text(id_);
...
bool AddFriendWindow::AddFriendClick(ui::EventArgs* param)
{
	...
	nim_ui::UserManager::GetInstance()->InvokeCheckAddFriend(userId, id_, ToWeakCallback([this](int code, const std::string& err_msg) {
		if (code == 0)
		{
			UTF8String msg = addfriend_msg_->GetUTF8Text();
			nbase::StringTrim(msg);

			nim::Friend::Request(id_, nim::kNIMVerifyTypeAdd, msg, ToWeakCallback([this](int res_code) {
				if (res_code == 200)
				{
		...
		{
			nim_ui::ShowToast(nbase::UTF8ToUTF16(err_msg), 2000, this->GetHWND());
		}
	}));
