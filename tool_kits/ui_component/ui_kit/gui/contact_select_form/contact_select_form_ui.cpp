...
const LPCSTR ContactSelectForm::kSendPersonCard = "SendPersonCard";
...
ContactSelectForm::ContactSelectForm(const UTF8String& uid_or_tid,
	...
	bool need_select_group/* = false*/, bool is_multi_vchat)
...
void ContactSelectForm::InitWindow()
{
	...
	else if (uid_or_tid_ == kSendPersonCard)
		title_id = L"STRING_INVITEUSERFORM_SEND_PERSONCARD";
	else
	...
	checkall_->AttachSelect(nbase::Bind(&ContactSelectForm::OnSelAllClick, this, std::placeholders::_1));
	checkall_->AttachUnSelect(nbase::Bind(&ContactSelectForm::OnSelAllClick, this, std::placeholders::_1));
...
//cjy
bool ContactSelectForm::OnSelAllClick(ui::EventArgs* param)
{
	...
	for (auto& it : tree_node_ver_)
	{
		for (int i = 0; i < it->GetCount(); i++)
		{
			auto listitem = (SelectedContactItemUI*)(it->GetItemAt(i));
			if (listitem)
			{
