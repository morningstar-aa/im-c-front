...
void ContactSelectForm::AddFriendListItem(const std::string& accid, bool is_enable)
{
	...
	ContactTileListUI* tile_layout = GetGroup(GT_COMMON_NUMBER);
	/*if (!ws_spell.empty())
	{
...
void ContactSelectForm::AddTeamListItem(const std::string& teamid, bool is_enable)
{
	...
	{ 
		if (*testiterator == teamid){
			return;
		}
	}
	ListContainerElement* contain = (ListContainerElement*)AddListItemInGroup(teamid, true, tile_layout,-1);
	if (contain)
	{
...
ui::Box* ContactSelectForm::AddListItemInGroup(const std::string& accid, bool is_team, ContactTileListUI* tile_layout,int k)
{
	...
	{
		tile_layout->AddAt(container_element, k);
	}
	else  
	{
		tile_layout->AddAt(container_element, k);
...
void ContactSelectForm::OnUserInfoChange(const std::list<nim::UserNameCard>& uinfos)
{
	...
	for (auto info : uinfos)
	{
		...
		if (user_service->GetUserType(accid) == nim::kNIMFriendFlagNormal && !MuteBlackService::GetInstance()->IsInBlackList(accid))
		{
			...
			if (friend_item != nullptr&&info.ExistValue(nim::kUserNameCardKeyIconUrl))
			{
