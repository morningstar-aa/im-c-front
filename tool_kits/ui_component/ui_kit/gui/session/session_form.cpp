...
namespace
{
	...
	const int kNormalSessionBoxWidth = 300;
	const int kNormalSessionBoxHeight = 400;
	const int kLargeSessionBoxWidth = 300;
	const int kLargeSessionBoxHeight = 400;
...
{
	if (param->chKey == 38 && param->Type == 9)//&& merge_list_->IsFocused())//向上
	{
		QLOG_APP(L"SessionForm::OnNotify param key is {0},type is {1}") << param->chKey << param->Type;
		int curSel = merge_list_->GetCurSel();
		if (curSel != 0)
		{
			MergeItem *merge_item = (MergeItem *)merge_list_->GetItemAt(curSel - 1);
			if (merge_item)
			{
				merge_item->Selected(true, false);
				std::wstring session_id = merge_item->GetName();
				ChangeToSessionBox(session_id);
			}
		}
	}
	else if (param->chKey == 40 && param->Type == 9)//&& merge_list_->IsFocused())//向下
	{
		QLOG_APP(L"SessionForm::OnNotify param key is {0},type is {1}") << param->chKey << param->Type;
		int curSel = merge_list_->GetCurSel();
		if (curSel < merge_list_->GetCount() - 1)
		{
			MergeItem *merge_item = (MergeItem *)merge_list_->GetItemAt(curSel + 1);
			if (merge_item)
			{
...
void SessionForm::CloseSessionBox(const std::string &session_id)
{
	...
	if (NULL != session_box)
	{
		...
		session_box->UninitSessionBox();
		// octodo: swap the simple and the independent
		SessionManager::GetInstance()->SwapSessionBox(session_id);
...
bool SessionForm::ChangeToSessionBox(const std::wstring &session_id, bool active/* = true*/)
{
	...
	{		
		...
		active_session_box_->OnMsgDown(NULL);
	}
	else
	{
