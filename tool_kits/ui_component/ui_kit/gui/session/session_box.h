...
#include "gui/session/control/bubbles/bubble_wallet.h"
#include "gui/session/control/bubbles/bubble_visitingcard.h"
...
#pragma region Init
	...
	* @return SessionForm* 会话窗口
	*/
	void GetSessionForm(ISessionDock* form);
...
public:
	...
	* 增加群添加消息，显示群公告
	*/
	void AddTeamMsg(const nim::IMMessage &msg, bool create);
	...
	*/
	...
	MsgBubbleItem* FindBubbleByMsgID(const std::string msgId);
...
public:
	...
	*/
	...
	/**
	* 设置发送按钮
	*@param[in] tag true设置为Ctrl+Enter发送，false设置Enter发送
	*/
	void setSendButtonStatus(bool tag);
	...
	* 发送设置菜单项被单机后的回调函数
	*/
	bool OnSendMenu(ui::EventArgs* arg);

	/**
	* 发送设置菜单项被单机后的回调函数
	*/
	void SettingSendForBox(bool isCtrl);	
...
private:
	...
	* @return void 无返回值
	*/
	void OnBtnPersonCard();

	void CardSelectedCompleted(const std::list<UTF8String>& friend_list, const std::list<UTF8String>& team_list, bool delete_enable);
	...
	*
	*/
	void AddCustomImage(const nim::IMMessage &md);

	/**
	* 上传收藏图片路径
	*
	*/
	void AddCustomImagePath(const std::string OriginalPath, const std::string thumbnailPath, curMsgInfo &info);
	
	/**
	* 删除收藏图片路径
	*
	*/
	void DeleteCustomImage(const std::wstring name);

	/**
	* 上传收藏文本
	*
	*/
	void AddBookmarksText(const std::string text, curMsgInfo &info);

	/**
	* 收藏文件
	*
	*/
	void AddBookmarksFileUpload(const nim::IMMessage &md);
	
	/**
	* 上传收藏图片路径
	*
	*/
	void AddFilePath(const std::string filePath, curMsgInfo &info);
	/**
	* 添加表情
	* 
	*/
	void AddCustomEmotion(const nim::IMMessage &md);

	/**
	* 删除自定义表情
	*
	*/
	void DeleteCustomEmotion(const std::wstring name);

	/**
	* 添加表情路径到服务器
	*
	*/
	void AddCustomEmotionPath(const std::string OriginalPath, const std::string thumbnailPath);

	/**
	* 群成员签到
	*
	*/
	void TeamNumberSignIn();

	/**
	* 群成员是否签到
	*
	*/
	void TeamNumberIsSignIn();

	/**
	* 导出群成员签到信息
	*
	*/
	void ExportNumberSignInInfo();

	/**
	* 选择导出签到数据时间回调函数
	*/
	void OnSearchSignInInfo(std::string startDate, std::string endDate, std::string filePath);
...
public:
	...
	*/
	...
	//下滚使用
	bool OnMsgDown(ui::EventArgs* param);
...
public:
	...
	*/
	...
	/**
	* 填充BoardView控件内容
	* @param[in] infos 群公告消息
	* @return void	无返回值
	*/
	void InitBoardView(std::vector<ForcePushManager::ForcePushInfo> &infos);
	...
	* @return void	无返回值
	*/
	void SelectAtUser(const std::string& uid, bool is_robot);
...
private:
	...
	*/
	...
	/**
	* 如果msg包含群公告消息，就添加到BoardView控件
	* @param[in] msg 消息
	* @return void	无返回值
	*/
	void AddBoardMessage(const nim::IMMessage &msg);
	...
	* @return void	无返回值
	*/
	void ForAddTeamAddBoardMessage(const nim::IMMessage &msg);	
	...
	* @return bool true 继续传递控件消息，false 停止传递控件消息
	*/
	bool OnBoardViewClick(ui::EventArgs* param);
	...
	*/
	...
	//下滚使用
	bool OnMsgDownClose(ui::EventArgs* param);
	...
	* @return bool true 继续传递控件消息，false 停止传递控件消息
	*/
	bool OnCloseBoardView(ui::EventArgs* param);
...
private:
	...
	* @return void	无返回值
	*/
	void SendPersonCard(const std::string id, const std::string name);
	...
	*/
	...
	/**
	* 发送一条小视频消息
	* @param[in] file_path 视频文件路径
	* @param[in] file_ext 语音文件扩展名
	* @return void	无返回值
	*/
	void SendMiniv(const std::wstring& file_path, const std::wstring& file_ext);
...
public:
	...
	* 设置群组禁言状态
	*/
	void setSessionMuteTag(bool tag);

	/**
	* 执行一键禁言群成员操作
	* @return void 无返回值
	*/
	bool MuteAllSession(bool isMute);
	...
	* @return void	无返回值
	*/
	static void OnTeamMuteEventCallback(const nim::TeamEvent& result);
...
private:
	...
	ui::Button*		btn_mute_all_;
	ui::Button*		btn_clear_mute_all_;
	bool            is_manager_;
	...
	customImgPath   imgPath_;//因回调函数作用域不同，为上传的图片返回地址记录全局变量

	customImgPath   localImgPathBack_;//因回调函数作用域不同，为图片原始地址记录全局变量

	curMsgInfo      curMsgInfo_;
	
	ui::RichEdit*	search_edit_;//群搜索
	ui::Button*		btn_clear_input_;
	ui::ListBox*	search_result_list_;
	//消息下滚逻辑
	int newmsg_count;
	ui::HBox* msg_down_;
	ui::Control* msg_down_close_;
	ui::Label*		msg_down_txt_;
	ui::Control*		msg_down_logo_;
private:
	// @功能相关变量
	...
	BoardView				*board_view_;			//群公告 消息预览
...
private:
	...
	std::string export_sign_path_;

	//导出签到数据的时间
	time_t export_begin_time_;
	time_t export_end_time_;

	std::string curClickId;
};
}