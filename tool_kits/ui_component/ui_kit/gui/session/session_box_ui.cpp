...
namespace nim_comp
{
	...
	void SessionBox::setSendButtonStatus(bool tag)
	{
		isSend_ = tag;
	}

	bool SessionBox::EditNotify(ui::EventArgs* param)
	{
		if (param->chKey == 17 && param->Type == 8)
		{
			isCtrl_ = true;
		}
		/*else if (param->chKey == 18)
		{
		isAlt_ = true;
		}*/
		else if (param->chKey == 13 && param->Type == 8)
		{
			if (isCtrl_)
			{
				if (isSend_)
			...
			else
			{
				...
				{
					//input_edit_->LineDown();
					input_edit_->SetSel(-1, -1);
					input_edit_->ReplaceSel(_T("\n"), true);
					input_edit_->SetFocus();
				}
				else
				{
		...
		/*if (param->Type != 31 && param->Type != 47 && param->Type != kEventKillFocus)
		{
		int i;
		i = param->Type;
		Type(kEventNone),
		...
		wParam(0),
		lParam(0)
		QLOG_ERR(L"1111111type is  ....{0}") << param->Type ;
		QLOG_ERR(L"2222222 key is  ....{0}") << param->chKey;
		QLOG_ERR(L"333333wKeyState is  ....{0}") << param->wKeyState;
		QLOG_ERR(L"444444wParam is  ....{0}") << param->wParam;
		QLOG_ERR(L"555555lParam is  ....{0}") << param->lParam;
		}*/
		return true;
	}
	...
	bool SessionBox::Notify(ui::EventArgs* param)
	{
		...
		else if (param->Type == ui::kEventNotify)
		{
			...
			else if (param->wParam == BET_RELOAD)
			{
				...
				item->SetLoadStatus(RS_LOADING);
				nim::NOS::FetchMedia(md, nbase::Bind(&SessionBox::OnRetweetResDownloadCallback, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), nim::NOS::ProgressCallback());

			}
			else if (param->wParam == BET_DELETE)
			{
				// need user confirm the delete action
				auto closure = [this, md](MsgBoxRet ret) {
					if (ret == MB_YES)
					{
						RemoveMsgItem(md.client_msg_id_);
				...
				ShowMsgBox(NULL, closure, L"STRID_SESSION_TIP_DELETE_MESSAGE", true, L"STRING_TIPS", true, L"STRING_OK", true, L"STRING_NO", true);
			}
			...
			else if (param->wParam == BET_RECALL)
			{
				...
				//std::string body;
				//body += "deleteMsgid=0";
				//if (md.session_type_ == nim::kNIMSessionTypeP2P)
				//{
				//	body += "type=7";
				//}
				//else
				...
				//	body += "type=8";
				//}
				//body += "&from=" + md.sender_accid_;
				//body += "&to=" + md.receiver_accid_;
				//body += "&delAccount=" + LoginManager::GetInstance()->GetAccount();
				//body += "&timetag=" + std::to_string(md.timetag_);
				//body += "&token=" + LoginManager::GetInstance()->GetAccountToken();

				//nim_http::HttpRequest request(app_sdk::AppSDKInterface::GetInstance()->GetAppHost() + "/api/other/recallMessage", body.c_str(), body.size(), function<void(bool, int, const std::string&)>([this, md](bool ret, int response_code, const std::string& reply) {
				//	Json::Value json;
				//	Json::Reader reader;
				//	bool res = reader.parse(reply, json);
				//	if (res)
				//	{
				//		if (json["code"].asString() == "1")
				//		{
				//			Post2UI(ToWeakCallback([this](){
				//				ShowMsgBox(this->GetWindow()->GetHWND(), MsgboxCallback(), L"STRID_SESSION_TIP_RECALL_TIMEOUT_MESSAGE", true);
				//			}));
				//		}
				//		else {
				//			nim::Talk::RecallMsg(md, "test notify when recall", nbase::Bind(&nim_comp::TalkCallback::OnReceiveRecallMsgCallback, std::placeholders::_1, std::placeholders::_2));
			...
			else if (param->wParam == BET_ADDEMOTION)
			{
				AddCustomEmotion(md);
			}
			else if (param->wParam == BET_ADDMSG)
			{
				if (md.type_ == nim::kNIMMessageTypeText)
				{
					curMsgInfo info;
					info.senderId = md.sender_accid_;
					info.senderName = md.readonly_sender_nickname_;
					//AddCustomImage(md);
					AddBookmarksText(md.content_, info);
				}
				else if (md.type_ == nim::kNIMMessageTypeImage)
				{
					AddCustomImage(md);
				}
				else if (md.type_ == nim::kNIMMessageTypeFile)
				{
					AddBookmarksFileUpload(md);
				}
			}
			else if (param->wParam == BET_SHOWPROFILE)
			{
				...
				if (session_type_ == nim::kNIMSessionTypeTeam)
				{
					...
					else
					{
						...
						if (type == nim::kNIMTeamUserTypeCreator || type == nim::kNIMTeamUserTypeManager)
						{
							ProfileForm::ShowProfileForm(session_id_, md.sender_accid_, type);
						}
						else
						{
							curClickId = md.sender_accid_;
							//从服务器获取能否添加好友
							nim_ui::UserManager::GetInstance()->InvokeGetMemberAddFriend(team_info_.GetTeamID(), ToWeakCallback([this](int code, const std::string &msg, const std::string &canAddFriend) {
								if (code == 0)
								{
									if (canAddFriend == "1")
									{
	...
	bool SessionBox::OnClicked(ui::EventArgs* param)
	{
		...
		else if (name == L"btn_add_member")
		{
			OnAddMember();
		}
		else if (name == L"btn_mute_all")
		{
			MuteAllSession(true);
		}
		else if (name == L"btn_clear_mute_all")
		{
			MuteAllSession(false);
		}
	...
	void SessionBox::PopupSendMenu()
	{
		...
		STRINGorID xml(L"sendsetting_menu.xml");
		pMenu->Init(xml, _T("xml"), point);

		CMenuElementUI* send_enter = (CMenuElementUI*)pMenu->FindControl(L"send_setting_enter");
		send_enter->AttachSelect(nbase::Bind(&SessionBox::OnSendMenu, this, std::placeholders::_1));
		Button* send_enter_btn = (Button*)send_enter->FindSubControl(L"send_setting_enter_btn");

		CMenuElementUI* send_ctrl = (CMenuElementUI*)pMenu->FindControl(L"send_setting_ctrl");
		send_ctrl->AttachSelect(nbase::Bind(&SessionBox::OnSendMenu, this, std::placeholders::_1));
		Button* send_ctrl_btn = (Button*)send_ctrl->FindSubControl(L"send_setting_ctrl_btn");

		if (isSend_)
		{
			send_enter_btn->SetBkImage(L"..\\menu\\send_set_this_no.png");
			send_ctrl_btn->SetBkImage(L"..\\menu\\send_set_this.png");
		}
		else
		{
			send_ctrl_btn->SetBkImage(L"..\\menu\\send_set_this_no.png");
			send_enter_btn->SetBkImage(L"..\\menu\\send_set_this.png");
		}
		pMenu->Show();
	}

	bool SessionBox::OnSendMenu(ui::EventArgs* arg)
	{
		std::wstring name = arg->pSender->GetName();
		if (name == L"send_setting_enter")
		{
			isSend_ = false;
			SettingSendForBox(false);
		}
		else if (name == L"send_setting_ctrl")
		{
			isSend_ = true;
			SettingSendForBox(true);
		}
		return false;
	}

	void SessionBox::SettingSendForBox(bool isCtrl)
	{
		string tag = "";
		if (isCtrl)
		{
		...
		std::wstring path = QPath::GetUserAppDataDir(nim_ui::LoginManager::GetInstance()->GetAccount());
		path += L"shortcuts.json";
	...
	void SessionBox::OnBtnImage(bool is_snapchat)
	{
		...
		LPCTSTR filter = L"*.jpg;*.jpeg;*.png;*.bmp;*.gif";
	...
	void SessionBox::OnBtnMiniv()
	{
		...
		CFileDialogEx::FileDialogCallback2 cb = nbase::Bind(&SessionBox::OnMinivSelected, this, std::placeholders::_1, std::placeholders::_2);
	...
	void SessionBox::OnImageSelected(bool is_snapchat, BOOL ret, std::wstring file_path)
	{
		if (ret)
		{
			...
			if (file_ext != L".jpg" && file_ext != L".jpeg" && file_ext != L".png" && file_ext != L".bmp" && file_ext != L".gif")
				return;
	...
	void SessionBox::OnMinivSelected(BOOL ret, std::wstring file_path)
	{
		if (ret)
		{
			if (!nbase::FilePathIsExist(file_path, FALSE) || 0 == nbase::GetFileSize(file_path))
	...
	void SessionBox::OnBtnEmoji()
	{
		...
		OnSelectSticker sel3 = nbase::Bind(&SessionBox::OnEmotionSelectedCustom, this, std::placeholders::_1, std::placeholders::_2);
		OnSelectSticker sel4 = nbase::Bind(&SessionBox::OnEmotionDeleteCustom, this, std::placeholders::_1, std::placeholders::_2);
		OnEmotionClose  cls = nbase::Bind(&SessionBox::OnEmotionClosed, this);

		EmojiForm* emoji_form = new EmojiForm;
		emoji_form->ShowEmoj(pt, sel, sel2, sel3, sel4, cls);
	}
	...
	void SessionBox::OnEmotionSelectedCustom(const std::wstring &catalog, const std::wstring &name)
	{
		//InsertCustomImgToEdit(input_edit_, file, userId);
	...
	void SessionBox::OnEmotionDeleteCustom(const std::wstring &catalog, const std::wstring &name)
	{
		/*std::string userId = nim_ui::LoginManager::GetInstance()->GetAccount();
		std::wstring file = QPath::GetUserAppDataDir(userId) + L"customImg\\custom_o\\" + name;
		if (nbase::FilePathIsExist(file, false))
	...
	void SessionBox::OnBtnPersonCard()
	{
		std::string wnd_id = ContactSelectForm::kSendPersonCard;
		...
		if (!contact_select_form)
		{
			...
			contact_select_form = new ContactSelectForm(wnd_id, exnclude_ids, nbase::Bind(&SessionBox::CardSelectedCompleted, this, std::placeholders::_1, std::placeholders::_2, true));
			contact_select_form->Create(NULL, L"", UI_WNDSTYLE_FRAME& ~WS_MAXIMIZEBOX, 0L);
			contact_select_form->CenterWindow();
		}
	...
	void SessionBox::CardSelectedCompleted(const std::list<UTF8String>& friend_list, const std::list<UTF8String>& team_list, bool delete_enable)
	{
		nim::TeamMemberProperty team_member;
		for (auto it = friend_list.begin(); it != friend_list.end(); it++)
		{
			std::string id = *it;
			std::wstring wname = UserService::GetInstance()->GetUserName(*it);
			std::string name = nbase::UTF16ToUTF8(UserService::GetInstance()->GetUserName(*it));
			SendPersonCard(id, name);
		}
