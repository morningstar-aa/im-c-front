...
namespace nim_comp
{
	...
	void MsgBubbleImage::InitInfo(const nim::IMMessage &msg)
	{
		...
		else //thumb_图片和原图都不存在
		{
			...
			{
				if (msg_.rescode_ == nim::kNIMResSuccess || msg_.rescode_ == nim::kNIMLocalResExist) 
				{
					SetLoadStatus(RS_LOADING);
				}
	...
	bool MsgBubbleImage::OnMenu(ui::EventArgs* arg)
	{
		...
		PopupMenu(true, true, show_retweet, true, true);
		return false;
	}
	...
	*/
	void MsgBubbleImage::OnMenuCopy()
	{
		...
		std::wstring dest = QPath::GetUserAppDataDir(userId) + L"copyImg\\";
		if (!nbase::FilePathIsExist(dest, true))
		{
			nbase::win32::CreateDirectoryRecursively(dest.c_str());
		}
		dest += L"temp.png";
		nbase::CopyFile(path_, dest);
		std::string path = nbase::UTF16ToUTF8(dest);
