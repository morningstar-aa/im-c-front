...
namespace nim_comp
{
	...
	void MsgBubbleItem::SetMsgStatus(nim::NIMMsgLogStatus status)
	{
		...
		case nim::kNIMMsgLogStatusSent:
			//status_receipt_->SetText(L"未读");
			//status_receipt_->SetVisible(true);
			break;

		case nim::kNIMMsgLogStatusReceipt:
			//status_receipt_->SetText(L"已读");
			//status_receipt_->SetVisible(true);
			break;
	...
	bool MsgBubbleItem::OnRightClick(ui::EventArgs* param)
	{
		...
		SessionBox* session_box = dynamic_cast<SessionBox*>(SessionManager::GetInstance()->FindSessionBox(msg_.local_talk_id_));
		if (session_box)
		{
			auto my_pro = session_box->GetTeamMemberInfo(LoginManager::GetInstance()->GetAccount());
			my_team_user_type = my_pro.GetUserType();
		}
		CMenuElementUI* banned_to_post = (CMenuElementUI*)pMenu->FindControl(L"banned_to_post");
		banned_to_post->AttachSelect(nbase::Bind(&MsgBubbleItem::OnMenu, this, std::placeholders::_1));

		CMenuElementUI* no_banned_to_post = (CMenuElementUI*)pMenu->FindControl(L"no_banned_to_post");
		no_banned_to_post->AttachSelect(nbase::Bind(&MsgBubbleItem::OnMenu, this, std::placeholders::_1));

		CMenuElementUI* kick_out_team = (CMenuElementUI*)pMenu->FindControl(L"kick_out_team");
		kick_out_team->AttachSelect(nbase::Bind(&MsgBubbleItem::OnMenu, this, std::placeholders::_1));

		std::wstring wid = nbase::UTF8ToUTF16(msg_.sender_accid_);

		ui::ListBox* member_list = session_box->getMemberList();

		TeamItem* item = dynamic_cast<TeamItem*>(member_list->FindSubControl(wid));
		isMute_ = false;
		if (item)
			isMute_ = item->SetMute();

		if (my_team_user_type == nim::kNIMTeamUserTypeCreator || my_team_user_type == nim::kNIMTeamUserTypeManager)
		{
			kick_out_team->SetVisible(true);

			if (isMute_)
			{
				no_banned_to_post->SetVisible(true);
			}
			else
			{
				banned_to_post->SetVisible(true);
			}
	...
	void MsgBubbleItem::HideAllStatus(int type)
	{
		try
		{
			if (type != 1)
			{
				if (status_loading_ != nullptr)
				{
					status_loading_->SetVisible(false);
				}
				if (status_reload_ != nullptr)
				{
					status_reload_->SetVisible(false);
				}
				if (status_load_failed_ != nullptr)
				{
			...
			if (type != 2)
			{
				if (status_sending_ != nullptr)
				{
					status_sending_->SetVisible(false);
				}
				if (status_resend_ != nullptr)
				{
					status_resend_->SetVisible(false);
				}
				if (status_send_failed_ != nullptr)
				{
					status_send_failed_->SetVisible(false);
				}
				if (status_receipt_ != nullptr)
				{
	...
	void MsgBubbleItem::PopupMenu(bool copy, bool recall, bool retweet/* = true*/, bool addemotion/* = false*/, bool addBookmarks/* = false*/)
	{
		if (!action_menu_)
			return;
		...
		rec->SetVisible(recall && IsShowRecallButton() && IsMyMsg());

		//if (IsMyMsg() && team_member_getter_ != nullptr)
		//{
		...
		//			// os=android&v=3.3.5&id=35040039191&token=abb8734d-c487-42ce-974b-a8bc62da01f5
		//			std::string body;
		//			body += "os=pc&v=" + app_sdk::AppSDKInterface::GetAppVersion();
		//			body += "&id=" + LoginManager::GetInstance()->GetAccount() + "&token=" + LoginManager::GetInstance()->GetAccountToken();

		//			nim_http::HttpRequest request(app_sdk::AppSDKInterface::GetInstance()->GetAppHost() + "/api/team/getTeamSetting", body.c_str(), body.size(), std::function<void(bool, int, const std::string&)>([this, pMenu, rec](bool result, int response_code, const std::string& reply) {
		//				Json::Value json;
		//				Json::Reader reader;
		//				bool res = reader.parse(reply, json);
		//				if (res)
		//				{
		//					rec->SetVisible(json["data"]["isrecall"].asString() == "1");
		//				}
		//				pMenu->Show();
		//			}));
		...
		addEmotion->AttachSelect(nbase::Bind(&MsgBubbleItem::OnMenu, this, std::placeholders::_1));
		addEmotion->SetVisible(addemotion);

		CMenuElementUI* add_Bookmarks = (CMenuElementUI*)pMenu->FindControl(L"add_bookmarks");
		add_Bookmarks->AttachSelect(nbase::Bind(&MsgBubbleItem::OnMenu, this, std::placeholders::_1));
		add_Bookmarks->SetVisible(addBookmarks);
		add_Bookmarks->SetVisible(false);
	}
	...
	bool MsgBubbleItem::OnMenu(ui::EventArgs* arg)
	{
		...
		else if (name == L"banned_to_post")
		{
			/*std::string tid = msg_.receiver_accid_;
			std::string talkid = msg_.local_talk_id_;
			std::string sid = msg_.sender_accid_;*/
			nim::Team::MuteMemberAsync(msg_.local_talk_id_, msg_.sender_accid_, !isMute_, nbase::Bind(&TeamCallback::OnTeamEventCallback, std::placeholders::_1));
			//m_pWindow->SendNotify(this, ui::kEventNotify, BET_BANNED_TO_POST, 0);
		}
		else if (name == L"no_banned_to_post")
		{
			nim::Team::MuteMemberAsync(msg_.local_talk_id_, msg_.sender_accid_, !isMute_, nbase::Bind(&TeamCallback::OnTeamEventCallback, std::placeholders::_1));
		}
		else if (name == L"kick_out_team")
		{
			std::list<std::string> uids_list;
			uids_list.push_back(msg_.sender_accid_);
			nim::Team::KickAsync(msg_.local_talk_id_, uids_list, nbase::Bind(&TeamCallback::OnTeamEventCallback, std::placeholders::_1));
			...
			//m_pWindow->SendNotify(this, ui::kEventNotify, BET_KICK_OUT_TEAM, 0);
		}
		return false;
	}
	...
	int MsgBubbleItem::CopyFileToClipboard(const char szFileName[])
	{
		UINT uDropEffect;
		HGLOBAL hGblEffect;
