...
void SessionBox::InitBoardView(std::vector<ForcePushManager::ForcePushInfo> &infos)
{
	for (auto& i : infos)
	{
		std::string sender_name = GetShowName(i.sender_id);
		if (!sender_name.empty())
		{
			...
			info.msg_body = i.msg_body;//nbase::UTF8ToUTF16(i.msg_body);
			info.uuid = nbase::UTF8ToUTF16(i.uuid);
			info.sender_name = nbase::UTF8ToUTF16(sender_name);
			board_view_->AddMessage(info);
		}
	}
}
...
void SessionBox::AddBoardMessage(const nim::IMMessage &msg)
{
	if (ForcePushManager::GetInstance()->IsBoardMsg(msg))
	{
		std::string sender_name = GetShowName(msg.sender_accid_);
		if (!sender_name.empty())
		{
			BoardView::BoardInfo info;
			info.msg_body = msg.attach_;//nbase::UTF8ToUTF16(msg.content_);
			info.uuid = nbase::UTF8ToUTF16(msg.client_msg_id_);
			info.sender_name = nbase::UTF8ToUTF16(sender_name);
			board_view_->AddMessage(info);
		}
...
void SessionBox::ForAddTeamAddBoardMessage(const nim::IMMessage &msg)
{
	std::string strBroad = edit_broad_->GetUTF8Text();//nbase::UTF8ToUTF16(msg.content_);
	if (strBroad.empty())
	{
		isBoardVisible_ = true;
		return;
	}
...
bool SessionBox::OnBoardViewClick(ui::EventArgs* param)
{
	if (param->pSender->GetName() != L"btn_board")
		return true;

	if (scroll_weak_flag_.HasUsed())
		return true;

	if (board_view_->GetAtMeCount() > 0)
	{
		MsgBubbleItem *cell = NULL;
		// 先查找当前窗体内是否有需要的消息
		std::wstring uuid = board_view_->GetUuid();
		for (int i = msg_list_->GetCount() - 1; i >= 0; --i)
		{
			if (uuid == msg_list_->GetItemAt(i)->GetName())
			{
...
bool SessionBox::OnCloseBoardView(ui::EventArgs* param)
{
	if (!board_view_)
		return false;

	board_view_->ShowNextMessage();
	board_view_->SetVisible(false);
...
bool SessionBox::HandleAtMsg(WPARAM wParam, LPARAM lParam)
{
	...
	if (wParam == '@')
	{
		...
		if (session_type_ == nim::kNIMSessionTypeTeam)
		{
			...
			at_list_form->SetTeamId(team_info_.GetTeamID());
...
void SessionBox::SelectAtUser(const std::string& uid, bool is_robot)
{
	if (uid.empty())
		return;
	...
	if (!LoginManager::GetInstance()->IsEqual(uid))
	{
		...
		input_edit_->GetSel(start, end);
		//std::wstring sel_str = input_edit_->GetTextRange(0, end);
		size_t pos = end - 1;//sel_str.rfind(L"@");
...
void SessionBox::OnSelectAtItemCallback(const std::string& uid)
{
	...
	if (!LoginManager::GetInstance()->IsEqual(uid))
	{
		...
		if ("000000000" == uid)
		{
			...
			int sendCount = PublicDB::GetInstance()->ReadForcePushOneData(userId, team_info_.GetTeamID(), date);
			std::string count = std::to_string(sendCount+1);
			PublicDB::GetInstance()->WriteForcePushData(userId, team_info_.GetTeamID(), date, count); 
		}
		else
		{
			da.uid_ = uid;
			...
			show_text = nbase::UTF8ToUTF16(show_name);
		}
