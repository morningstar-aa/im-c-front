...
AtlistForm::AtlistForm(std::string session_id, nim::NIMSessionType session_type, std::string tid, OnSelectAtItem cb)
{
...
void AtlistForm::InitWindow()
{
	...
	m_pRoot->AttachBubbledEvent(kEventSelect, nbase::Bind(&AtlistForm::OnSelectItem, this, std::placeholders::_1));
...
void AtlistForm::InitTeamMembers(const std::map<std::string, nim::TeamMemberProperty>& team_member_info_list)
{
	...
	//at all 设置id为000000000
	std::string uid = LoginManager::GetInstance()->GetAccount();
	auto it_info = team_member_info_list.find(uid);
	if (it_info != team_member_info_list.end())
	{
		std::string allStr = "000000000";
		AtListItem *member = AddListItem(allStr, 0, false);
		member->SetHeadImage(L"../public/header/head_team.png");
...
nim_comp::AtListItem* AtlistForm::AddListItem(const std::string& uid, int index, bool is_last_five)
{
	...
	{
		if (index == 1&&team_members_container_->GetCount()>0)
		{
...
nim_comp::AtListItem* AtlistForm::CreateAtListItem(const std::string& uid, bool is_last_five)
{
	...
	if (session_type_ == nim::kNIMSessionTypeTeam)
	{
		...
		if ("000000000" == uid)
		{
			item->SetTeamCardName(L"all(所有人)");
		}
	}
...
bool AtlistForm::Match(const std::wstring& search_key, bool match_visible)
{
	...
	for (int i = uid_last_five_.size(); i < count; i++)
	{
		...
		std::wstring show_name = item->GetShowName();
		if (L"all(所有人)" == show_name)
		{
			time_t timep;
			time(&timep);
			char date_c[16];

			strftime(date_c, sizeof(date_c), "%Y%m%d", localtime(&timep));
			std::string date = date_c;
			std::string userId = LoginManager::GetInstance()->GetAccount();
			int sendCount = PublicDB::GetInstance()->ReadForcePushOneData(userId, teamId_, date);
			std::map<std::string, std::string> settingMap = LoginManager::GetInstance()->GetClientSetting();
			auto user_type = team_member_info_list_[item->GetUserID()].GetUserType();
			if ((user_type == nim::kNIMTeamUserTypeCreator || user_type == nim::kNIMTeamUserTypeManager || settingMap["iseveryman"] == "1"))
			{
				if (user_type == nim::kNIMTeamUserTypeCreator || user_type == nim::kNIMTeamUserTypeManager)
				{
					item->SetVisible(true);
				}
				else if (atoi(settingMap["everyonecount"].c_str()) - sendCount > 0)
				{
					item->SetVisible(true);
				}
				else
...
void AtlistForm::SetTeamId(std::string tid)
{
	teamId_ = tid;
}
...
void AtlistForm::ShowMemberItems(std::list<std::string> &last_five)
{
	...
	{
		AtListItem *member = AddListItem(*it, 1, true);
		ASSERT(member != NULL);
	}*/
...
void AtlistForm::OnUserInfoChange(const std::list<nim::UserNameCard> &uinfos)
{
	for (auto info : uinfos)
	{
		...
		// 同时更新最近5个发言人里的子项的昵称 cjy
		AtListItem* five_item = NULL; FindInLastFiveSender(wuid);
