...
#include "gui/session/control/board_view.h"
...
namespace
{
	const int kMsgLogNumberShow = 50;
}
...
namespace nim_comp
{
	...
	SessionBox::SessionBox(std::string id, nim::NIMSessionType type)
	{
		...
		isSend_ = nim_comp::ShortcutsManager::GetInstance()->GetSendShortcutsStatus();

		/*if (session_type_ == nim::kNIMSessionTypeTeam)
		{
		std::string userId = nim_ui::LoginManager::GetInstance()->GetAccount();

		sendCount_ = PublicDB::GetInstance()->ReadForcePushOneData(userId, team_info_.GetTeamID(),);
		}*/
	}
	...
	void SessionBox::GetSessionForm(ISessionDock* form)
	{
		session_form_ = form;
	}
	...
	void SessionBox::InitSessionBox()
	{
		...
		//label_ip = (Label*)FindSubControl(L"label_ip");
		//label_ip->SetText(L"127.0.0.1");
		...
		msg_list_->AttachScrollChange(nbase::Bind(&SessionBox::OnScrollChange, this, std::placeholders::_1));
		...
		btn_add_member_ = (Button*)FindSubControl(L"btn_add_member");
		//btn_add_member_->SetVisible(true);
		btn_mute_all_ = (Button*)FindSubControl(L"btn_mute_all");
		btn_mute_all_->SetEnabled(false);
		btn_clear_mute_all_ = (Button*)FindSubControl(L"btn_clear_mute_all");
		...
		//下滚逻辑 
		msg_down_ = (HBox*)(FindSubControl(L"msg_down"));
		msg_down_logo_ = (Control*)(FindSubControl(L"msg_down_logo"));
		msg_down_close_ = (Control*)(FindSubControl(L"msg_down_close"));
		msg_down_txt_ = (Label*)(FindSubControl(L"msg_down_txt"));
		msg_down_logo_->AttachButtonDown(nbase::Bind(&SessionBox::OnMsgDown, this, std::placeholders::_1));
		msg_down_txt_->AttachButtonDown(nbase::Bind(&SessionBox::OnMsgDown, this, std::placeholders::_1));
		msg_down_close_->AttachButtonDown(nbase::Bind(&SessionBox::OnMsgDownClose, this, std::placeholders::_1));

		//群公告控件初始化
		board_view_ = new BoardView;
		board_view_->InitControl();
		msg_content_->Add(board_view_);
		board_view_->AttachBubbledEvent(kEventClick, nbase::Bind(&SessionBox::OnBoardViewClick, this, std::placeholders::_1));
		Button* button_close_board = static_cast<Button*>(board_view_->FindSubControl(L"btn_close_board"));
		button_close_board->AttachClick(nbase::Bind(&SessionBox::OnCloseBoardView, this, std::placeholders::_1));

		//搜索
		search_edit_ = static_cast<RichEdit*>(FindSubControl(L"search_edit"));
		...
		btn_clear_input_->AttachClick(nbase::Bind(&SessionBox::OnClearInputBtnClicked, this, std::placeholders::_1));
		btn_clear_input_->SetNoFocus();

		search_result_list_ = static_cast<ui::ListBox*>(FindSubControl(_T("search_result_list")));
		...
		AtlistForm *at_list_form = new AtlistForm(session_id_, session_type_, team_info_.GetTeamID(), ToWeakCallback(cb));
	...
	//下滚 cjy
	bool SessionBox::OnMsgDown(ui::EventArgs* param)
	{
		newmsg_count = 0;
		msg_down_->SetVisible(false);
		//msg_list_->EndDown(true, false);
		return true;
	}

	bool SessionBox::OnMsgDownClose(ui::EventArgs* param)
	{
		msg_down_->SetVisible(false);
		return true;
	}
	bool SessionBox::OnScrollChange(ui::EventArgs* param)
	{
		if (newmsg_count > 0){
			if (msg_list_->IsAtEnd()){
				OnMsgDown(param);
			}
		}
		return true;
	...
	void SessionBox::AddNewMsg(const nim::IMMessage &msg, bool create)
	{
		...
		{
			std::string id = GetSessionId();
			if (msg.session_type_ == nim::kNIMSessionTypeTeam)
			{
				if (!SessionManager::GetInstance()->IsTeamMsgNotify(id, msg.sender_accid_))
					flash = false;
			}
			else
			{
				flash = !MuteBlackService::GetInstance()->IsInMuteList(id);
			}
		...
		//else//cjy 添加新消息标注
		//{
		//	newmsg_count = newmsg_count + 1;
		//	msg_down_txt_->SetText(nbase::IntToString16(newmsg_count) + L"条新消息");
		//	msg_down_->SetVisible(true);
		//}
	...
	void SessionBox::AddTeamMsg(const nim::IMMessage &msg, bool create)
	{
		//bool at_end = msg_list_->IsAtEnd();
	...
	MsgBubbleItem* SessionBox::ShowMsg(const nim::IMMessage &msg, bool first, bool show_time)
	{
		...
		else if (msg.type_ == nim::kNIMMessageTypeNotification || msg.type_ == nim::kNIMMessageTypeTips)
		{
			...
			if (StringToJson(msg.attach_, json) && json.isObject())
			{
				...
				{
					if (!json[nim::kNIMNotificationKeyData][nim::kNIMNotificationKeyTeamInfo]["mute_all"].isObject())
					{
						return nullptr;
					}
				}
				else if (id == nim::kNIMNotificationIdTeamKick && !LoginManager::GetInstance()->IsEqual(msg.sender_accid_))
				{
		...
		else if (msg.type_ == nim::kNIMMessageTypeCustom)
		{
			...
			if (StringToJson(msg.attach_, json) && json.isObject())
			{
				...
				else if (sub_type == CustomMsgType_Wallet_send)
				{
					MsgBubbleWallet *walletItem = new MsgBubbleWallet;
					walletItem->SetWalletType(sub_type);
					item = walletItem;
				}
				else if (sub_type == CustomMsgType_Visiting_Card)
				{
					MsgBubbleVisitingCard *visitingcardItem = new MsgBubbleVisitingCard;
					//visitingcardItem->InitControl()
					//visitingcardItem->InitInfo(msg);
					item = visitingcardItem;
				}
				else if (sub_type == CustomMsgType_Wallet_receive)
				{
					return nullptr;
				}
			}
		...
		//// cache func
		//int msgCount_max = msg.session_type_ == nim::kNIMSessionTypeTeam ? 100 : 70;
		//while (msg_list_->GetCount() > msgCount_max)
		//{
		//	MsgBubbleItem* item_c = (MsgBubbleItem*)cached_msgs_bubbles_.front();
		//	for (size_t i = 0; i < msg_list_->GetCount(); i++)
		//	{
		//		MsgBubbleItem* item_v = (MsgBubbleItem*)msg_list_->GetItemAt(i);
		//		if (item_c == item_v)
		//		{
		//			msg_list_->RemoveAt(i);
		//			nim::IMMessage itemMsg = item_v->GetMsg();
		//			id_bubble_pair_.erase(itemMsg.client_msg_id_);
		//			break;
		//		}
		//	}
		//	cached_msgs_bubbles_.pop_front();
		//}
	...
	MsgBubbleItem* SessionBox::FindBubbleByMsgID(const std::string msgID){
		IdBubblePair::iterator it = id_bubble_pair_.find(msgID);
		if (it != id_bubble_pair_.end())
		{
			MsgBubbleItem* item = it->second;
			if (item)
	...
	void SessionBox::SendText(const std::string &text, bool team_msg_need_ack/* = false*/)
	{
		...
		if (!uid_at_someone_.empty())
		{
			...
			for (auto it = uid_at_someone_.begin(); it != uid_at_someone_.end(); ++it)
			{
				...
				if (index != std::string::npos)
				{
					if (!it->second.is_robot_)
					{
						...
						if ((it->first == "all") && (it->second.uid_ == "000000000"))
						{
							isAtAll = true;
						}
						else
			...
			if (!msg.msg_setting_.force_push_ids_list_.empty() || isAtAll)
			{
		...
		if (msg.type_ != nim::kNIMMessageTypeRobot)
		{
			...
			encrypt_enc->SetEncryptKey(key);
			encrypt_enc->SetEncryptIvParameterSpec(iv);
			encrypt_enc->EnableEncryptPadding(true, 7);
			UTF8String password_aes;
			encrypt_enc->Encrypt(msg.content_, password_aes);
			std::string base641;
			nbase::Base64Encode(password_aes, &base641);
			msg.content_ = base641;
	...
	void SessionBox::SendPersonCard(const std::string id, const std::string name)
	{
		nim::IMMessage msg;
		Json::Value values;
		Json::Value json;
		json["type"] = CustomMsgType_Visiting_Card;
		values["content"] = name;
		values["personCardId"] = id;
		if (session_type_ == nim::kNIMSessionTypeP2P)
		{
			...
			values["title"] = nbase::UTF16ToUTF8(title);
		}
		else
		{
			std::wstring title = ui::MutiLanSupport::GetInstance()->GetStringViaID(L"STRID_SESSION_VISITING_CARD_MSG_TITLE");
			values["title"] = nbase::UTF16ToUTF8(title);
		}
	...
	void SessionBox::SendMiniv(const std::wstring& wfile_path, const std::wstring& wfile_ext)
	{
		try{
			std::string file_path = nbase::UTF16ToUTF8(wfile_path);
			std::string file_ext = nbase::UTF16ToUTF8(wfile_ext);
			...
			nim::IMVideo video;
			video.md5_ = GetFileMD5(wfile_path);
			video.size_ = (long)nbase::GetFileSize(wfile_path);

			nbase::PathString file_name;
			nbase::FilePathApartFileName(wfile_path, file_name);
			video.display_name_ = nbase::UTF16ToUTF8(file_name);
			...
			MediaInfo MI;
			MI.Open(wfile_path);
			String width = MI.Get(stream_t::Stream_Video, 0, _T("Width")).c_str();
			String height = MI.Get(stream_t::Stream_Video, 0, _T("Height")).c_str();
			String duration = MI.Get(stream_t::Stream_Video, 0, _T("Duration")).c_str();
			MI.Close();


			video.width_ = atoi(nbase::UTF16ToUTF8(width).c_str());
			video.height_ = atoi(nbase::UTF16ToUTF8(height).c_str());
			video.duration_ = atoi(nbase::UTF16ToUTF8(duration).c_str());
			...
			QLOG_ERR(L"send mini video error: {0}.") << nbase::UTF8ToUTF16(e.what());
		}
	}
	...
	void SessionBox::AddCustomEmotionPath(const std::string OriginalPath, const std::string thumbnailPath)
	{
		imgPath_.OriginalPath = OriginalPath;
		imgPath_.thumbnailPath = thumbnailPath;
		std::string userId = nim_ui::LoginManager::GetInstance()->GetAccount();
		nim_ui::UserManager::GetInstance()->InvokeUploadImgPath(userId, OriginalPath, thumbnailPath, ToWeakCallback([this](int code, const std::string imgId) {
			if (code == 0)
			{
				if (imgId.empty())
				{
					//弹框展示失败
					ShowMsgBox(this->GetWindow()->GetHWND(), MsgboxCallback(), L"STRID_SESSIONBOX_CUSTOMEMOTION_FAIL", true);
				}
				else
				{
					//弹框显示成功
					//保存图片路径（包括缩略图路径与原图路径）与id到文件
					std::string userId = nim_ui::LoginManager::GetInstance()->GetAccount();

					CustomData data;
					data.user_id_ = userId;
					data.img_id_ = imgId;
					data.OriginalPath = imgPath_.OriginalPath;
					data.thumbnailPath = imgPath_.thumbnailPath;
					PublicDB::GetInstance()->WriteCustomData(data);
					//更新缩略图列表
					/*string url_o = imgPath_.OriginalPath;
					...
					string url = imgPath_.thumbnailPath;
					...
					ShowMsgBox(this->GetWindow()->GetHWND(), MsgboxCallback(), L"STRID_SESSIONBOX_CUSTOMEMOTION_SUCCESS", true);
				}
			}
			else
			{
	...
	long URLDownloadToLikeImg(std::string url, std::wstring name)
	{
		size_t len = url.length();//获取字符串长度
		int nmlen = MultiByteToWideChar(CP_ACP, 0, url.c_str(), len + 1, NULL, 0);//如果函数运行成功，并且cchWideChar为零，
	...
	void SessionBox::AddCustomEmotion(const nim::IMMessage &md)
	{
		bool b = PublicDB::GetInstance()->AddRecord("shoucang", md.client_msg_id_);
		if (b == true){
			std::string strPath = "";
			if (!md.local_res_path_.empty())
			{
				strPath = md.local_res_path_ + ".png";
				localImgPathBack_.OriginalPath = md.local_res_path_;
				localImgPathBack_.thumbnailPath = strPath;
			}
			else
			{
				Json::Value json;
				if (StringToJson(md.attach_, json) && json.isObject())
				{
					Json::Value jsonValue = json["data"];
					if (jsonValue.isObject())
					{
						//strPath = jsonValue["chartlet"].asString();
						//if (std::string::npos != strPath.find("http://"))
						{
							std::string userId = nim_ui::LoginManager::GetInstance()->GetAccount();
							std::string url = jsonValue["chartlet"].asString();
							...
							{
								URLDownloadToLikeImg(url, namePath);
							}
							strPath = nbase::UTF16ToUTF8(namePath);
							localImgPathBack_.OriginalPath = strPath;
							localImgPathBack_.thumbnailPath = strPath;
						}
					}
				}
			}
			/*if (std::string::npos == strPath.find(".png") && std::string::npos != strPath.find("like_buf"))
			{
			ShowMsgBox(this->GetWindow()->GetHWND(), MsgboxCallback(), L"STRID_SESSIONBOX_CUSTOMIMAGE_FAIL_SYSTEM", true);
			return;
			}*/
			rename(md.local_res_path_.data(), strPath.data());

			nim_ui::UserManager::GetInstance()->InvokeUploadImg(strPath, ToWeakCallback([this](int code, const std::string OriginalPath, const std::string thumbnailPath) {

				rename(localImgPathBack_.thumbnailPath.data(), localImgPathBack_.OriginalPath.data());
				if (code == 0)
				{
					if (OriginalPath.empty() || thumbnailPath.empty())
					{
						//弹框展示失败
						ShowMsgBox(this->GetWindow()->GetHWND(), MsgboxCallback(), L"STRID_SESSIONBOX_CUSTOMEMOTION_FAIL", true);
					}
					...
					{
						AddCustomEmotionPath(OriginalPath, thumbnailPath);
					}
				}
				else
				{
			...
			ShowMsgBox(this->GetWindow()->GetHWND(), MsgboxCallback(), L"已收藏，请不要重复收藏", false);
			return;
		}
	}

	void SessionBox::DeleteCustomEmotion(const std::wstring name)
	{
		//删除用户保存的本地图片文件
		std::string userId = nim_ui::LoginManager::GetInstance()->GetAccount();
		std::wstring wFile_o = QPath::GetUserAppDataDir(userId) + L"like\\" + name;
		std::string file_o = nbase::UTF16ToUTF8(wFile_o);
		if (nbase::FilePathIsExist(wFile_o, false))
		{
			std::remove(file_o.data());
		}
		/*std::wstring Wfile = QPath::GetUserAppDataDir(userId) + L"like\\" + name;
		std::string file = nbase::UTF16ToUTF8(Wfile);
		if (nbase::FilePathIsExist(Wfile, false))
		{
		std::remove(file.data());
		}*/

		//删除本地数据库中记录的数据
		std::string name_utf8 = nbase::UTF16ToUTF8(name);
		std::string imgId = name_utf8.substr(0, name_utf8.length() - 4);
		...
		//向服务器发送请求 删除服务器数据
		nim_ui::UserManager::GetInstance()->InvokeDeleteImgPath(userId, imgId, ToWeakCallback([this](int code, const std::string msg) {
			if (code == 0)
			{
				//弹框展示成果
				ShowMsgBox(this->GetWindow()->GetHWND(), MsgboxCallback(), L"STRID_SESSIONBOX_CUSTOMEMOTION_DEL_SUCCESS", true);
			}
			else
			{
				//弹框展示失败
				ShowMsgBox(this->GetWindow()->GetHWND(), MsgboxCallback(), L"STRID_SESSIONBOX_CUSTOMEMOTION_DEL_FAIL", true);
			}
		}));
	}

	void SessionBox::DeleteCustomImage(const std::wstring name)
	{
		//删除用户保存的本地图片文件
		std::string userId = nim_ui::LoginManager::GetInstance()->GetAccount();
		std::wstring wFile_o = QPath::GetUserAppDataDir(userId) + L"customImg\\custom_o\\" + name;
		std::string file_o = nbase::UTF16ToUTF8(wFile_o);
		if (nbase::FilePathIsExist(wFile_o, false))
		{
			std::remove(file_o.data());
		}
		std::wstring Wfile = QPath::GetUserAppDataDir(userId) + L"customImg\\custom\\" + name;
		std::string file = nbase::UTF16ToUTF8(Wfile);
		if (nbase::FilePathIsExist(Wfile, false))
		{
			std::remove(file.data());
	...
	void SessionBox::AddCustomImagePath(const std::string OriginalPath, const std::string thumbnailPath, curMsgInfo &info)
	{
		imgPath_.OriginalPath = OriginalPath;
		imgPath_.thumbnailPath = thumbnailPath;
		std::string userId = nim_ui::LoginManager::GetInstance()->GetAccount();
		curBookmarksInfo curInfo;
		curInfo.uid = userId;
		curInfo.type = 1;
		curInfo.path = OriginalPath;
		curInfo.imgCompPath = thumbnailPath;
		curInfo.senderId = info.senderId;
		curInfo.senderName = info.senderName;
		nim_ui::UserManager::GetInstance()->InvokeUploadBookmarks(curInfo, ToWeakCallback([this](int code, const std::string imgId) {
			if (code == 0)
			{
				if (imgId.empty())
				{
					//弹框展示失败
					ShowMsgBox(this->GetWindow()->GetHWND(), MsgboxCallback(), L"STRID_SESSIONBOX_CUSTOMIMAGE_FAIL", true);
				}
				else
				{
					//为收藏图片到表情包保留代码
					...
					if (!nbase::FilePathIsExist(dir_o, true) || !nbase::FilePathIsExist(dir, true))
					{
					...
					ShowMsgBox(this->GetWindow()->GetHWND(), MsgboxCallback(), L"STRID_SESSIONBOX_CUSTOMIMAGE_SUCCESS", true);
				}
			}
			else
			{
	...
	void SessionBox::AddCustomImage(const nim::IMMessage &md)
	{
		std::string strPath = md.local_res_path_ + ".png";
		rename(md.local_res_path_.data(), strPath.data());
		localImgPathBack_.OriginalPath = md.local_res_path_;
		localImgPathBack_.thumbnailPath = strPath;
		curMsgInfo_.senderId = md.sender_accid_;
		curMsgInfo_.senderName = md.readonly_sender_nickname_;
		...
		nim_ui::UserManager::GetInstance()->InvokeUploadImg(strPath, ToWeakCallback([this](int code, const std::string OriginalPath, const std::string thumbnailPath) {
			...
			if (code == 0)
			{
				...
				{
					AddCustomImagePath(OriginalPath, thumbnailPath, curMsgInfo_);
				}
			}
			else
			{
	...
	void SessionBox::AddFilePath(const std::string filePath, curMsgInfo &info)
	{
		std::string userId = nim_ui::LoginManager::GetInstance()->GetAccount();
		curBookmarksInfo curInfo;
		curInfo.uid = userId;
		curInfo.type = 6;
		curInfo.path = filePath;
		curInfo.senderId = info.senderId;
		curInfo.senderName = info.senderName;
	...
	void SessionBox::AddBookmarksFileUpload(const nim::IMMessage &md)
	{
		//std::string strPath = md.local_res_path_ + ".png";
		//rename(md.local_res_path_.data(), strPath.data());
		//localImgPathBack_.OriginalPath = md.local_res_path_;
		...
		if (StringToJson(md.attach_, json) && json.isObject())
		{
			filePath = json["url"].asString();
		}

		nim_ui::UserManager::GetInstance()->InvokeUploadFile(filePath, ToWeakCallback([this](int code, const std::string filePath, const std::string thumbnailPath) {
			...
			if (code == 0)
			{
				...
				{
					AddFilePath(filePath, curMsgInfo_);
				}
			}
			else
			{
	...
	void SessionBox::AddBookmarksText(const std::string text, curMsgInfo &info)
	{
		std::string userId = nim_ui::LoginManager::GetInstance()->GetAccount();
		curBookmarksInfo curInfo;
		curInfo.uid = userId;
		...
		nim_ui::UserManager::GetInstance()->InvokeUploadBookmarks(curInfo, ToWeakCallback([this](int code, const std::string imgId) {
			if (code == 0)
			{
				...
				else
				{
					...
					if (!nbase::FilePathIsExist(dir_o, true) || nbase::FilePathIsExist(name_o, false)
					|| !nbase::FilePathIsExist(dir, true) || nbase::FilePathIsExist(name, false))
					{
					ShowMsgBox(this->GetWindow()->GetHWND(), MsgboxCallback(), L"STRID_SESSIONBOX_CUSTOMIMAGE_FAIL", true);
					return;
					}
	...
	void SessionBox::TeamNumberSignIn()
	{
		//btn_sign_in_->SetEnabled(false);
		std::string userId = nim_ui::LoginManager::GetInstance()->GetAccount();
		std::string teamId = team_info_.GetTeamID();

		nim_ui::UserManager::GetInstance()->InvokeSignIn(userId, teamId, ToWeakCallback([this](int code, const std::string msg) {
			if (code == 0)
			{
				//弹框展示失败
				ShowMsgBox(this->GetWindow()->GetHWND(), MsgboxCallback(), L"STRID_TEAM_SIGN_IN_SUCCESS", true);
			}
			else
			{
				//弹框展示失败
				ShowMsgBox(this->GetWindow()->GetHWND(), MsgboxCallback(), L"STRID_TEAM_SIGN_IN_FAIL", true);
				//btn_sign_in_->SetEnabled(true);
			}
		}));
	}

	void SessionBox::TeamNumberIsSignIn()
	{
		//btn_sign_in_->SetEnabled(false);
		std::string userId = nim_ui::LoginManager::GetInstance()->GetAccount();
		std::string teamId = team_info_.GetTeamID();

		nim_ui::UserManager::GetInstance()->InvokeIsSignIn(userId, teamId, ToWeakCallback([this](int code, const std::string msg, bool isSign) {
			if (code == 0)
			{
				if (!isSign)
				{
	...
	void SessionBox::ExportNumberSignInInfo()
	{
		OnSearchSignDataCallBack cb = nbase::Bind(&SessionBox::OnSearchSignInInfo, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
		nim_ui::WindowsManager::SingletonShow<ExportSignForm>(ExportSignForm::kClassName);
		ExportSignForm *instence = (ExportSignForm *)nim_ui::WindowsManager::GetInstance()->GetWindow(ExportSignForm::kClassName, ExportSignForm::kClassName);
		instence->SetCallBack(cb);
	}
	...
	void SessionBox::OnSearchSignInInfo(std::string startDate, std::string endDate, std::string filePath)
	{
		//btn_sign_in_export_->SetEnabled(false);
		//std::string userId = nim_ui::LoginManager::GetInstance()->GetAccount();
		std::string teamId = team_info_.GetTeamID();
		export_sign_path_ = filePath;
		string temp_begin_time = startDate + "000000";
		export_begin_time_ = FormatTime2(temp_begin_time.c_str());
		string temp_end_time = endDate + "000000";
		export_end_time_ = FormatTime2(temp_end_time.c_str());

		nim_ui::UserManager::GetInstance()->QueryTeamSignIn(teamId, startDate, endDate, ToWeakCallback([this](int code, const std::string msg, const std::list<SignInfo> &infoList) {

			if (0 == code)
			{
				ofstream oFile;

				if (export_sign_path_.empty())
				{
					ShowMsgBox(this->GetWindow()->GetHWND(), MsgboxCallback(), L"STRID_TEAM_SIGN_IN_EXPORT_FAIL", true);
					return;
				}
				oFile.open(export_sign_path_.data(), ios::out | ios::trunc);
				//oFile << "日期" << "," << "用户ID" << "," << "昵称" << "," << "签到" << endl;
				//oFile << "日期" << "," << "昵称" << "," << "签到" << endl;
				oFile << "日期" << "," << "帐号" << "," << "昵称" << "," << "签到" << endl;
				//只导出选择日期签到数据（包含account）
				auto iterDateSign = infoList.begin();
				for (; iterDateSign != infoList.end(); iterDateSign++)
				{
					std::string date = iterDateSign->signDate;
					auto iter = team_member_info_list_.begin();
					for (; iter != team_member_info_list_.end(); iter++)
					{
						std::string uId = iter->first;
						nim::TeamMemberProperty teamMemberInfo = iter->second;
						std::string nick = teamMemberInfo.GetNick();
						if (nick.empty())
						{
							nick = nbase::UTF16ToUTF8(UserService::GetInstance()->GetUserName(uId));
						}
						std::string isSign = "否";
						/*auto iterSign = iterDateSign->idList.begin();
						for (; iterSign != iterDateSign->idList.end(); iterSign++)
						{
						if (uId == *iterSign)
						{
						isSign = "是";
						...
						std::string account = "";
						std::map<std::string, std::string> tempMap = iterDateSign->idMap;
						if (tempMap.end() != tempMap.find(uId))
						{
							isSign = "是";
							account = tempMap[uId];
						}
						std::wstring wnick = nbase::UTF8ToUTF16(nick);
						//char szChar[64] = "";
						int ansiLen = ::WideCharToMultiByte(CP_ACP, NULL, wnick.data(), wcslen(wnick.data()), NULL, 0, NULL, NULL);
						//同上，分配空间要给'\0'留个空间
						char* szAnsi = new char[ansiLen + 1];
						//转换
						//unicode版对应的strlen是wcslen
						::WideCharToMultiByte(CP_ACP, NULL, wnick.data(), wcslen(wnick.data()), szAnsi, ansiLen, NULL, NULL);
						//最后加上'\0'
						szAnsi[ansiLen] = '\0';
						if (isSign == "是")
						{
							oFile << date.data() << "," << account.data() << "," << szAnsi << "," << isSign.data() << endl;
						}
						//oFile << date.data() << "," << szAnsi << "," << isSign.data() << endl;
						delete[]szAnsi;
					}
				}
				////导出所有选择日期的签到数据到excel
				/*string begin_time = FormatTime(export_begin_time_);
				begin_time = begin_time.substr(0, 8);
				string end_time = FormatTime(export_end_time_);
				end_time = end_time.substr(0, 8);
				...
				oFile << date.data() << "," << uId.data() << "," << szAnsi << "," << isSign.data() << endl;
				//oFile << date.data() << "," << szAnsi << "," << isSign.data() << endl;
				delete[]szAnsi;
				}
				export_begin_time_ = export_begin_time_ + 60 * 60 * 24;
				begin_time = FormatTime(export_begin_time_);
				begin_time = begin_time.substr(0, 8);
				}*/
				//导出有签到日期的数据到excel
				/*auto iterDateSign = infoList.begin();
				...
				ShowMsgBox(this->GetWindow()->GetHWND(), MsgboxCallback(), L"STRID_TEAM_SIGN_IN_EXPORT_SUCCESS", true);
			}
			else
			{
				ShowMsgBox(this->GetWindow()->GetHWND(), MsgboxCallback(), L"STRID_TEAM_SIGN_IN_EXPORT_FAIL", true);
			}
			//btn_sign_in_export_->SetEnabled(true);
		}));
	}
	...
	void SessionBox::RemoveAllMsgItem()
	{
	...
	void SessionBox::CheckHeader()
	{
		...
		else
		{
			...
			/*UserService::GetInstance()->QueryOtherUserInfo(session_id_, ToWeakCallback([this](int code, const std::string msg, const UserBaseInfo userInfo) {
				if (code == 0)
				{
				label_ip->SetText(L"最后登录：【" + nbase::UTF8ToUTF16(userInfo.ip) + L"】 " + nbase::UTF8ToUTF16(userInfo.address));
				}
				}));*/
			// 设置ip
			//std::string szIp = UserService::GetInstance()->GetUserCustom(session_id_).asString();
			//replace_str(szIp, "\"", "");
			//ip = nbase::UTF8ToUTF16(szIp);
		}
