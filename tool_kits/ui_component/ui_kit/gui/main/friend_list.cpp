	...
	pos_tip_(nullptr)
{
	...
	//	return true;
	//});
	add_friend_item->AttachClick(nbase::Bind(&FriendList::AddFriendClick, this, std::placeholders::_1));
...
bool FriendList::AddFriendClick(ui::EventArgs* param)
{
	std::string userId = LoginManager::GetInstance()->GetAccount();
	nim_ui::UserManager::GetInstance()->InvokeCheckAddFriendAuth(userId, ToWeakCallback([this](int code, const std::string& err_msg) {
		if (code == 0)
		{
			WindowsManager::GetInstance()->SingletonShow<AddFriendWindow>(AddFriendWindow::kClassName);
		}
		...
		{
			nim_ui::ShowToast(nbase::UTF8ToUTF16(err_msg), 2000);
		}
	}));
