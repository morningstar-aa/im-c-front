...
void TeamEventItem::InitCtrl()
{
	...
	//this->AttachDoubleClick(nbase::Bind(&TeamEventItem::OnDbClicked, this, std::placeholders::_1));
...
bool TeamEventItem::InitInfo(const nim::SysMessage &json)
{
	...
	else if (msg_type_ == nim::kNIMSysMsgTypeFriendAdd)
	{
		...
		else if (verify_type == nim::kNIMVerifyTypeAgree)
		{
			evt_tip_->SetText(mls->GetStringViaID(L"STRID_MSG_CENTER_ADD_YOU_FRIEND_AGREED"));
		}
		else if (verify_type == nim::kNIMVerifyTypeReject)
		{
			evt_tip_->SetText(mls->GetStringViaID(L"STRID_MSG_CENTER_ADD_YOU_FRIEND_REFUSED"));
		}
	}
	else
	{
...
//bool TeamEventItem::OnDbClicked(ui::EventArgs* arg)
//{
//	SessionManager::GetInstance()->OpenSessionBox(acc_id_, nim::kNIMSessionTypeP2P);
//	return true;
//}
...
bool TeamEventItem::OnClicked(ui::EventArgs* arg)
{
	...
	if (name == L"btn_ok")
	{
		...
		else if (msg_type_ == nim::kNIMSysMsgTypeFriendAdd)
		{
			...
			nim_ui::UserManager::GetInstance()->InvokeAddFriend(userId, acc_id_, ToWeakCallback([this](int code, const std::string &msg) {
				if (code == 0)
				{
				}
				else
	...
	else if (name == L"btn_head")
	{
		if (msg_type_ == nim::kNIMSysMsgTypeFriendAdd) {
			SessionManager::GetInstance()->OpenSessionBox(acc_id_, nim::kNIMSessionTypeP2P);
		}
		else {
			SessionManager::GetInstance()->OpenSessionBox(tid_, nim::kNIMSessionTypeTeam);
		}
	}
	return false;
}
