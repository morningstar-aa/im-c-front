...
void FriendItem::Init(FriendItemType type, const std::string &accid)
{
	...
	if (type_ != kFriendItemTypeRobot)
	{
		...
		else
		{
			...
			remark_name_=UserService::GetInstance()->GetFriendAlias(id_);
			if (remark_name_ != L""){
				remark_name_ = nbase::MakeLowerString(remark_name_);
				remark_name_full_spell_ = nbase::MakeLowerString(PinYinHelper::GetInstance()->ConvertToFullSpell(remark_name_));
				remark_name_simple_spell_ = nbase::MakeLowerString(PinYinHelper::GetInstance()->ConvertToSimpleSpell(remark_name_));
			}
		}
	}
	else
...
bool FriendItem::Match(const UTF8String& search_key)
{
	...
	if (nick_name_.find(ws_search_key) != std::wstring::npos
		...
		|| remark_name_.find(ws_search_key) != std::wstring::npos
		|| remark_name_full_spell_.find(search_key) != UTF8String::npos
		|| remark_name_simple_spell_.find(search_key) != UTF8String::npos
		|| id_.find(search_key) != UTF8String::npos)
	{
		return true;
	}
	return false;
