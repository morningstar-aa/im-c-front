...
typedef std::function<void(ui::ListContainerElement *)> OnTopItemCallback;
...
public:
	...
	* @return void	无返回值
	*/
	bool InitMsg(const nim::SessionData &msg);
	...
	* @return void	无返回值
	*/
	void UpdateMsgContent(bool &isContinue, const std::string& id = "");
	...
	*/
	...
	bool DeleteItemRecentSession(ui::EventArgs* param);

	/**
	* 从最近联系人删除
	*/
	void DeleteLatestContact();
	/*
	**设置置顶回调换上
	*/
	void setTopItemCallBack(OnTopItemCallback fun_topItem_cb, OnTopItemCallback fun_cancel_topItem_cb);

	bool GetIsTop(){ return isTop_; };
	void SetIsTop(bool isTop, const nim::SessionData &item_data);

	bool SetIsTopEx(const nim::SessionData &msg);
private:
	/**
	* 显示或隐藏@我消息提示
	* @param[in] show 显示或隐藏
	...
	* @return bool true 继续传递控件消息，false 停止传递控件消息
	*/
	bool SessionItemDoubleClick(ui::EventArgs* param);
	...
	*/
	...
	static void OnSessionTopCb(nim::NIMResCode code, const nim::SessionData& data, int total_unread_counts);

private:
	ui::ButtonBox*	head_image_;
	ui::Label*		label_name_;
	ui::Label*		label_msg_;
	ui::Label*		label_msg_redpacket_;
	...
	bool isTop_;
	OnTopItemCallback topItem_cb_;
	OnTopItemCallback cancel_topItem_cb_;
};
