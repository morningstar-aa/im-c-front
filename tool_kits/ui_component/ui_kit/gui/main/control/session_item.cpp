...
namespace nim_comp
{
	...
	void SessionItem::InitCtrl()
	{
		...
		label_msg_redpacket_ = (Label*) this->FindSubControl(L"label_msg_redpacket");
		...
		//this->AttachDoubleClick(nbase::Bind(&SessionItem::SessionItemDoubleClick, this, std::placeholders::_1));
	}

	bool SessionItem::InitMsg(const nim::SessionData &msg)
	{
		msg_ = msg;
		...
		UpdateMsgContent(isContinue); //更新消息内容
		if (isContinue)
		{
			return true;
		}
	...
	void SessionItem::UpdateMsgContent(bool &isContinue, const std::string& id /*= ""*/)
	{
		...
		if (msg_.msg_status_ != nim::kNIMMsgLogStatusDeleted)
		{
			...
			if (show_text == L"群已被解散" || show_text == L"你 离开了群" || show_text == L"你 已被移出群")
			{
			...
			/*else if (show_text == L"你 离开了群")
			{
			}
			else if (show_text == L"你 已被移除群")
			{
			}*/
			if (show_text == L"[打开红包消息]")
			{
				return;
			}

			if (show_text == L"他人 已被移出群" || show_text == L"[自定义消息]")//不显示
			{
				return;
			}
			/*else if (show_text.find(L"设置了全员禁言") || show_text.find(L"解除了全员禁言") || show_text.find(L"禁言") || show_text.find(L"解除禁言"))
			{
			return;
			}*/
		...
		if (show_text == L"[红包消息]")
		{
			label_msg_redpacket_->SetVisible(true);
			label_msg_->SetVisible(false);
		}
		else
		{
			label_msg_->SetText(show_text);
			label_msg_redpacket_->SetVisible(false);
			label_msg_->SetVisible(true);
		}
	}
	...
	void SessionItem::OnSessionTopCb(nim::NIMResCode code, const nim::SessionData& data, int total_unread_counts)
	{
		QLOG_APP(L"session top, code={0} command={1} total_un_cn={2}") << code << data.command_ << total_unread_counts;
	}

	void SessionItem::setTopItemCallBack(OnTopItemCallback fun_topItem_cb, OnTopItemCallback fun_cancel_topItem_cb)
	{
		topItem_cb_ = fun_topItem_cb;
		cancel_topItem_cb_ = fun_cancel_topItem_cb;
	}

	bool SessionItem::SetIsTopEx(const nim::SessionData &msg)
	{
		SessionItemTopData data;
		data.uid = LoginManager::GetInstance()->GetAccount();
		data.accid = msg.id_;
		PublicDB::GetInstance()->GetSessionItemTopData(data);
		isTop_ = data.value;
		if (isTop_)
		{
			FindSubControl(L"top_icon_name")->SetVisible(true);
		}
		else
		{
			FindSubControl(L"top_icon_name")->SetVisible(false);
		}
		return true;
	}

	void SessionItem::SetIsTop(bool isTop, const nim::SessionData &item_data)
	{
		isTop_ = isTop;
		SessionItemTopData data;
		data.uid = LoginManager::GetInstance()->GetAccount();
		data.accid = item_data.id_;
		data.value = isTop;
		bool b = PublicDB::GetInstance()->WriteSessionItemTopData(data);
		if (isTop)
		{
			FindSubControl(L"top_icon_name")->SetVisible(true);
		}
	...
	void SessionItem::BatchStatusDeleteCb(nim::NIMResCode res_code, const std::string& uid, nim::NIMSessionType to_type)
	{
		...
		SessionBox *session_box = SessionManager::GetInstance()->FindSessionBox(uid);
		if (session_box)
			session_box->RemoveAllMsgItem();
	}
	...
	void SessionItem::PopupSessionItemMenu(POINT point)
	{
		...
		CMenuElementUI* detach_session_item = (CMenuElementUI*)pMenu->FindControl(L"detach_session_item");
		detach_session_item->AttachSelect(nbase::Bind(&SessionItem::DelSessionItemMenuItemClick, this, std::placeholders::_1));
		detach_session_item->SetVisible(false);

		CMenuElementUI* top_item = (CMenuElementUI*)pMenu->FindControl(L"top_item");
		top_item->AttachSelect(nbase::Bind(&SessionItem::DelSessionItemMenuItemClick, this, std::placeholders::_1));

		CMenuElementUI* cancel_top_item = (CMenuElementUI*)pMenu->FindControl(L"cancel_top_item");
		...
		// ctodo: open independent
		CMenuElementUI* open_independent = (CMenuElementUI*)pMenu->FindControl(L"open_independent");
		open_independent->AttachSelect(nbase::Bind(&SessionItem::DelSessionItemMenuItemClick, this, std::placeholders::_1));
	...
	bool SessionItem::DeleteItemRecentSession(ui::EventArgs* param)
	{
	...
	void SessionItem::DeleteLatestContact()
	{
		nim::Session::DeleteRecentSession(msg_.type_, msg_.id_, nbase::Bind(&SessionItem::DeleteRecentSessionCb, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
		SubscribeEventManager::GetInstance()->UnSubscribeSessionEvent(msg_);
		m_pWindow->SendNotify(this, ui::kEventNotify, SET_DELETE, 0);
	...
	bool SessionItem::SessionItemDoubleClick(ui::EventArgs* param)
	{
		bool has_transfer_task = TransferFileManager::GetInstance()->HasTransferTask(msg_.id_);
	...
	bool SessionItem::DelSessionItemMenuItemClick(ui::EventArgs* param)
	{
		...
		else if (name == L"detach_session_item")
		{
			bool has_transfer_task = TransferFileManager::GetInstance()->HasTransferTask(msg_.id_);
		...
		{
			//nim::Session::DeleteRecentSession(msg_.type_, msg_.id_, nbase::Bind(&SessionItem::DeleteRecentSessionCb, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
			//nim::Session::SetSessionTop(msg_.type_, msg_.id_, true, nbase::Bind(&SessionItem::OnSessionTopCb, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
			nim::Session::SetSessionTop(msg_.type_, msg_.id_, true, nbase::Bind(&SessionItem::OnSessionTopCb, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
			/*nim::Session::SetUnreadCountZeroAsync(msg_.type_, msg_.id_, ToWeakCallback([this](nim::NIMResCode res_code, const nim::SessionData&, int){
				if (res_code == nim::kNIMResSuccess)
				nim::MsgLog::BatchStatusDeleteAsync(msg_.id_, msg_.type_, nbase::Bind(&SessionItem::BatchStatusDeleteCb, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
			...
			if (topItem_cb_)
			{
				Post2UI(nbase::Bind(topItem_cb_, this));
			}
		}
		else if (name == L"cancel_top_item")
		{
			//nim::Session::DeleteRecentSession(msg_.type_, msg_.id_, nbase::Bind(&SessionItem::DeleteRecentSessionCb, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
			nim::Session::SetSessionTop(msg_.type_, msg_.id_, false, nbase::Bind(&SessionItem::OnSessionTopCb, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
			/*nim::Session::SetUnreadCountZeroAsync(msg_.type_, msg_.id_, ToWeakCallback([this](nim::NIMResCode res_code, const nim::SessionData&, int){
			if (res_code == nim::kNIMResSuccess)
			nim::MsgLog::BatchStatusDeleteAsync(msg_.id_, msg_.type_, nbase::Bind(&SessionItem::BatchStatusDeleteCb, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
			}));
			Post2UI(nbase::Bind(LoginCallback::OnMultispotChange, true, login_res.other_clients_));*/
			if (cancel_topItem_cb_)
			{
				Post2UI(nbase::Bind(cancel_topItem_cb_, this));
			}
		}
		else if (name == L"open_independent") {
			// get opened window
			SessionBox* sessionBox = SessionManager::GetInstance()->FindSessionBox(msg_.id_);
			if (sessionBox)
			{
				sessionBox->SetVisible(false);
			}
			SessionManager::GetInstance()->OpenSessionBox(msg_.id_, msg_.type_, false, true);
		}
