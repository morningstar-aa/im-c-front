...
public:
	...
	*/
	...
	std::list<std::string> GetAllSession(bool qun);
	/**
	* 添加一个会话控件到会话列表
	* @param[in] msg 会话数据
	...
	* item置顶
	*/
	SessionItem* SetCurItemTop(const nim::SessionData &msg);

	/**
	* item取消置顶
	*/
	SessionItem* SetCurItemCancelTop(const nim::SessionData &msg);
	/**
	* 从会话列表查找会话控件
	...
	*/
	...
	/**
	* 响应上下按钮sessionlist上下切换
	*/
	void UpDownSessionItem(bool flag);

	bool DetachSessionItem(const std::string &session_id, ui::EventArgs* item_param);

	void onItemTop(ui::ListContainerElement *item);
	void onCancelItemTop(ui::ListContainerElement *item);

	/**
	* 从最近联系人删除
	*/
	void DeleteLatestContact(std::string team_id);
	/*
	**获取当前用户session_box
	*/
...
private:
	...
	*/
	...
	//bool OnAllEventsClick(ui::EventArgs* param);
