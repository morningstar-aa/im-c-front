...
public:
	...
	* @param[in] close_cb 选中收藏表情的回调函数
	* @param[in] close_cb 窗口的回调函数
	* @param[in] only_emoj 是否只显示表情
	* @return void	无返回值
	*/
	void ShowEmoj(POINT pt, OnSelectEmotion sel, OnSelectSticker sel_sticker, OnSelectSticker sel_custom, OnSelectSticker del_custom, OnEmotionClose close_cb, bool only_emoj = false);
...
private:
	...
	* @return void	无返回值
	*/
	void AddSticker(std::wstring name, std::vector<std::string>& files, bool type = false);
	...
	* @return bool true 继续传递控件消息，false 停止传递控件消息
	*/
	bool OnCustomClicked(ui::EventArgs* param);
	...
	*/
	...
	bool OnRightHandMenu(ui::EventArgs* arg);
...
private:
	...
	OnSelectSticker sel_custom_cb_;
	OnSelectSticker del_custom_cb_;
	OnEmotionClose	close_cb_;

	bool			only_emoj_;
	bool			is_closing_;
	std::wstring     del_imgId_;
};
}