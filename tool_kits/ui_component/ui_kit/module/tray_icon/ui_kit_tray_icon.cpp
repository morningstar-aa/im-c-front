...
#define WM_TRAYICON_NOTIFY		(WM_USER + 1)
	...
	static int msg_tag = 0;
	static std::wstring showMsg = L"";
	static HICON showIcon = NULL;
		...
		case uTrayIconAnimateID:
			if (0 != anim_index_array_.size())
			{
				if (msg_tag)
				{
					...
					nim_ui::UserManager::GetInstance()->GetUserName(my_id, false);
					SetTrayIcon(::LoadIconW(nbase::win32::GetCurrentModuleHandle(), MAKEINTRESOURCE(105)), ui::MutiLanSupport::GetInstance()->GetStringViaID(L"STRID_MIANWINDOW_TITLE") + L"\n昵称:" + LoginManager::GetInstance()->GetAccount());
					*/	//SetTrayIconIndex(anim_index_array_[msg_tag]);
					msg_tag = 0;
				}
				else
				{
				...
				SetTrayIcon(::LoadIconW(nbase::win32::GetCurrentModuleHandle(), MAKEINTRESOURCE(106)), ui::MutiLanSupport::GetInstance()->GetStringViaID(L"STRID_MIANWINDOW_TITLE"));
			}
			else
			{
				msg_tag = 0;
			...
			{
				msg_tag = 0;
				SetTrayIcon(::LoadIconW(nbase::win32::GetCurrentModuleHandle(), MAKEINTRESOURCE(105)), ui::MutiLanSupport::GetInstance()->GetStringViaID(L"STRID_MIANWINDOW_TITLE"));
				StopTrayIconAnimate();
				return;
			}
	...
	void TrayIcon::SetTrayIconIndex(int index)
	{
		...
		assert(hIcon);
		SetTrayIcon(hIcon, ui::MutiLanSupport::GetInstance()->GetStringViaID(L"STRID_MIANWINDOW_TITLE"));
	}
		...
		HICON hTrayIcon, const std::wstring &tray_icon_text)
	{
		...
		if (tray_icon_text.find('\n') != std::wstring::npos)
		{
			showIcon = hTrayIcon;
			showMsg = tray_icon_text;
		}
		if (msg_tag == 0){
			tnd.hIcon = (showIcon == NULL ? hTrayIcon : showIcon);
		}
		else{
			tnd.hIcon = NULL;
		}
		wcscpy_s(tnd.szTip, 128, showMsg.c_str());
		tnd.uCallbackMessage = dwTrayMsg;
		if (!bInit)
		{
			bInit = true;
