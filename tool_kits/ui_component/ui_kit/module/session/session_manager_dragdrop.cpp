...
void SessionManager::SetSendButStatus(bool enable)
{
	...
	for (auto it_box : session_box_map_)
	{
		...
		SessionBox *parent_form = it_box.second;
		parent_form->setSendButtonStatus(enable);
	}
}
...
void SessionManager::SetEnableMerge(bool enable)
{
	...
	else
	{
		...
		for (auto it_box : session_box_map_)
		{
			...
			{
				ISessionDock *session_form = ISessionDock::InstantDock(false);
