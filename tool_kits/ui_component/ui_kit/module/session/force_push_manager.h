...
public:
	...
	* @return bool true 包含，false 不包含
	*/
	bool IsBoardMsg(const nim::IMMessage &msg);
	...
	* @return bool true 包含，false 不包含
	*/
	bool IsAddTeamMsg(const nim::IMMessage &msg, std::string &id);
	...
	* @return bool true 包含，false 不包含
	*/
	bool IsPassiveAddTeamMsg(const nim::IMMessage &msg);
	...
	* @return void 无返回值
	*/
	void AddBoardMsg(const std::string &session_id, const nim::IMMessage &msg);
	...
	* @return bool true 成功，false 失败
	*/
	bool GetBoardMsgs(const std::string &session_id, std::vector<ForcePushInfo> &infos);
	...
	* @return void 无返回值
	*/
	void ResetBoardMsg(const std::string &session_id);
	...
	*/
	...
	/**
	* 查找某个会话中是否包含未读的群公告的消息
	* @param[in] session_id 会话id
	* @return bool true 包含，false 不包含
	*/
	bool IsContainBoard(const std::string &session_id);
...
private:
	...
	//begin add by gx
	std::map<std::string, std::vector<ForcePushInfo>> session_id_board_;// 回话是否包含未读的群公告 
	//end add by gx
};
}