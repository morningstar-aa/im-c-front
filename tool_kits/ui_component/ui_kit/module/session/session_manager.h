...
public:
	...
	*/
	SessionBox* OpenSessionBox(std::string session_id, nim::NIMSessionType type, bool reopen = false, bool independent = false);
	...
	* @return SessionBox* 返回会话盒子的指针
	*/
	SessionBox* OpenSessionBoxEx(std::string session_id, nim::NIMSessionType type, bool reopen = false);
	/*激活当前对话框
	*/
	SessionBox* OpenCurrentSessionBox();

	/*
	*获取当前回话盒子
	*/
	SessionBox* GetCurrentSessionBox();
	...
	* @return void 无返回值
	*/
	void AddTeamMsg(const nim::IMMessage &msg);
	...
	*/
	...
	SessionBox* FindSessionBoxEx(const std::string &id);

	void SwapSessionBox(const std::string &id);
...
public:
	...
	* 设置声音选项
	*/
	void SetSpeakerStatus(bool isSpeaker);

	/**
	* 设置是发送快捷键
	*/
	void SetSendButStatus(bool enable);
...
private:
	...
	*/
	SessionBox* CreateSessionBox(const std::string &session_id, nim::NIMSessionType type, bool independent);
	...
	* @return SessionBox*	会话盒子指针
	*/
	SessionBox* CreateSessionBoxEx(const std::string &session_id, nim::NIMSessionType type);
...
private:
	...
	std::map<std::string, SessionBox*>	session_box_map_ind_;
