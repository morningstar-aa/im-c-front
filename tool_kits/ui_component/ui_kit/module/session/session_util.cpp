...
std::wstring GetMessageTimePart(const long long t) {
	assert(t > 0);
	if (t <= 0)
		return L"";
...
void GenerateUploadImage( const std::wstring &src, const std::wstring &dest )
{
	...
	else
	{
		...
		zoom.SetAutoZoom(true, 640, 640);//cjy3
...
bool InsertCustomImgToEdit(ui::RichEdit* edit, const std::wstring &file_name, const std::string &id)
{
	bool ret = false;

	std::wstring file = QPath::GetUserAppDataDir(id) + L"customImg\\custom_o\\" + file_name;
	//file.append(L"res\\emoji\\" + file_name);
	if (nbase::FilePathIsExist(file, false))
	{
		ITextServices* service = edit->GetTextServices();
		ret = Re_InsertFace(service, file, file_name);
		if (!ret)
		{
			QLOG_ERR(L"insert emoj {0} {1} fail") << file_name << file;
		}
		service->Release();
	}
	else
	{
		QLOG_ERR(L"emoj {0} {1} miss") << file_name << file;
	}
	return ret;
}
		...
		else
		{
			...
			if (!array.empty() && array.isArray())
			{
				...
				if (!ids.empty())
				{
					...
					{
						//将文本设置为他人被踢出群，为屏蔽该条信息做比较
						if (obj != you && !LoginManager::GetInstance()->IsEqual(from_account))
							obj = mls->GetStringViaID(L"STRID_SESSION_ITEM_OTHER");
...
std::wstring GetCustomMsg(const std::string &sender_accid, const std::string &msg_attach)
{
	...
	if (StringToJson(msg_attach, json) && json.isObject())
	{
		...
		{
			show_text = mls->GetStringViaID(L"STRID_SESSION_ITEM_WALLET_MSG");
		}
		else if (sub_type == CustomMsgType_Visiting_Card)
		{
			show_text = mls->GetStringViaID(L"STRID_SESSION_ITEM_VISITING_CARD_MSG");
		}
		else if (sub_type == CustomMsgType_Wallet_receive)
		{
			show_text = mls->GetStringViaID(L"STRID_SESSION_ITEM_WALLET_MSG_OPEN");
		}
	}
	return show_text;
}
