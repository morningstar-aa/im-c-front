...
bool Re_InsertQQTextInfo(ITextServices * text_service, std::string& nim_text, bool custom, std::function<void()> callback)
{
	if (text_service)
	{
		if (nim_text.size() > 0)
		{
			...
			doc.Parse(nim_text.c_str());
			//test
			TiXmlNode* parent = doc.FirstChild("QQRichEditFormat");
			if (parent)
			{
				for (TiXmlNode* node = parent->FirstChild(); node != NULL; node = node->NextSibling())
				{
...
void Re_InsertHtmlInfo(ITextServices * text_service, std::string& nim_text, bool custom, std::function<void()> callback)
{
	if (text_service)
	{
		if (nim_text.size() > 0)
		{
			std::string strStartHtml, strEndHtml;
			int startHtml = nim_text.find("StartHTML");
			if (std::string::npos != startHtml)
			{
				strStartHtml = nim_text.substr(startHtml + 10, 10);
			}

			int endHtml = nim_text.find("EndHTML");
			if (std::string::npos != endHtml)
			{
				strEndHtml = nim_text.substr(endHtml + 8, 10);
			}
			startHtml = std::stoi(strStartHtml);
			endHtml = std::stoi(strEndHtml);
			std::string strHtml = nim_text.substr(startHtml, endHtml - startHtml);

			/*std::wstring szfilename = L"c:/1.txt";
			std::ofstream OsWrite(szfilename, std::ofstream::app);
			OsWrite << strHtml;
			OsWrite << std::endl;
			OsWrite.close();*/

			/*std::string strStartFragment, strEndFragment;
			int startFragment = nim_text.find("StartFragment");
			if (std::string::npos != startFragment)
			{
				strStartFragment = nim_text.substr(startFragment + 14, 10);
			}

			int endFragment = nim_text.find("EndFragment");
			if (std::string::npos != endFragment)
			{
				strEndFragment = nim_text.substr(endFragment + 12, 10);
			}
			startFragment = std::stoi(strStartFragment);
			endFragment = std::stoi(strEndFragment);
			std::string strFragment = nim_text.substr(startFragment, endFragment - startFragment);*/

			CMarkup xml_Path;
			std::wstring wstrHtml = nbase::UTF8ToUTF16(strHtml);
			xml_Path.SetDoc(wstrHtml.data());
			if (!xml_Path.FindElem(_T("html")))
			{
				return;
			}
			xml_Path.IntoElem();
			if (!xml_Path.FindElem(_T("body")))
			{
				return;
			}
			xml_Path.IntoElem();
			while (xml_Path.FindElem(_T("p")))
			{
				xml_Path.IntoElem();
				while (xml_Path.FindElem(_T("span")))
				{
					int text_limit_surplus = Re_GetTextLimitSurplus(text_service);
					if (text_limit_surplus <= 0)
					{
					...
					bool limit_out = false;
					if (xml_Path.FindChildElem(_T("img")))
					{
						std::wstring tmp = xml_Path.GetChildAttrib(_T("src"));
						std::wstring delStr = L"file:///";
						if (std::wstring::npos != tmp.find(delStr))
						{
							tmp = tmp.substr(delStr.length(), tmp.length() - delStr.length());
						}
						//Re_InsertFile(text_service, callback, tmp);
						Re_InsertImage(text_service, callback, true, tmp);
					}
					std::wstring wtmp = xml_Path.GetData();
					int text_add = wtmp.length();
					if (true)
					{
...
void Re_InsertScreenshotInfo(ITextServices * text_service, std::wstring& nim_text, bool custom, std::function<void()> callback)
{
	Re_InsertImage(text_service, callback, true, nim_text);
}

void Re_InsertFileInfo(ITextServices * text_service, std::wstring& nim_text, bool custom, std::function<void()> callback)
{
	if (std::wstring::npos != nim_text.find(L".png") 
		|| std::wstring::npos != nim_text.find(L".jpg")
		|| std::wstring::npos != nim_text.find(L".jpeg")
		|| std::wstring::npos != nim_text.find(L".gif")
		|| std::wstring::npos != nim_text.find(L".bmp"))
	{
		Re_InsertImage(text_service, callback, true, nim_text);
	}
	else
	{
		Re_InsertFile(text_service, callback, nim_text);
	}
}
