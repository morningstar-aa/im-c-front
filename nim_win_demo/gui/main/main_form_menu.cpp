...
void MainFormMenu::OnPopupMainMenu(POINT point)
{
	...
	CMenuElementUI* shortcuts_setting = (CMenuElementUI*)pMenu->FindControl(L"shortcuts_setting");
	shortcuts_setting->AttachSelect(nbase::Bind(&MainFormMenu::ShortcutsSettingMenuItemClick, this, std::placeholders::_1));

	CMenuElementUI* group_send = (CMenuElementUI*)pMenu->FindControl(L"group_send_tool");
	group_send->AttachSelect(nbase::Bind(&MainFormMenu::GroupSendMenuItemClick, this, std::placeholders::_1));

	//CMenuElementUI* bookmarks = (CMenuElementUI*)pMenu->FindControl(L"bookmarks");
	//bookmarks->AttachSelect(nbase::Bind(&MainFormMenu::BookmarksMenuItemClick, this, std::placeholders::_1));
	...
	CMenuElementUI* speaker_setting = (CMenuElementUI*)pMenu->FindControl(L"speaker_setting");
	speaker_setting->AttachSelect(nbase::Bind(&MainFormMenu::SpeakerSettingMenuItemClick, this, std::placeholders::_1));
	if (is_speaker_)
	{
		((Button *)speaker_setting->FindSubControl(L"speaker_setting_btn"))->SetBkImage(L"icon_speaker_open_normal.png");
		((Label *)speaker_setting->FindSubControl(L"speaker_setting_label"))->SetText(MutiLanSupport::GetInstance()->GetStringViaID(L"STRID_MAINWINDOW_MENU_SPEAKER_CLOSE_SETTING"));
	}
	else
	{
		((Button *)speaker_setting->FindSubControl(L"speaker_setting_btn"))->SetBkImage(L"icon_speaker_close_normal.png");
		((Label *)speaker_setting->FindSubControl(L"speaker_setting_label"))->SetText(MutiLanSupport::GetInstance()->GetStringViaID(L"STRID_MAINWINDOW_MENU_SPEAKER_OPEN_SETTING"));
	}
...
bool MainFormMenu::ShortcutsSettingMenuItemClick(ui::EventArgs* param)
{
	//TODO
	nim_ui::WindowsManager::GetInstance()->ShowShortcutsSettingForm();
	return true;
}

bool MainFormMenu::GroupSendMenuItemClick(ui::EventArgs* param)
{
	//TODO
	nim_ui::WindowsManager::GetInstance()->ShowGroupSendForm();
	return true;
}

bool MainFormMenu::BookmarksMenuItemClick(ui::EventArgs* param)
{
	//TODO
	nim_ui::WindowsManager::GetInstance()->ShowBookmarksForm();
	return true;
}

bool MainFormMenu::SpeakerSettingMenuItemClick(ui::EventArgs* param)
{
	is_speaker_ = !is_speaker_;
	//TODO
	nim_comp::SessionManager::GetInstance()->SetSpeakerStatus(is_speaker_);

	//写配置文件
	std::string acc = nim_ui::LoginManager::GetInstance()->GetAccount();
	std::wstring theme_dir = QPath::GetUserAppDataDir(acc);
	std::wstring path = theme_dir + L"setting.json";
	std::string str = nbase::UTF16ToUTF8(path);

	ifstream infile;
	infile.open(str.data(), ios::app);   //将文件流对象与文件连接起来 
	if (!infile.is_open())
	{
		return true;
	}
...
void MainFormMenu::IntSysSetting()
{
	std::string acc = nim_ui::LoginManager::GetInstance()->GetAccount();
	std::wstring theme_dir = QPath::GetUserAppDataDir(acc);
	std::wstring path = theme_dir + L"setting.json";
	...
	if (reader.parse(strJsonData, root))
	{
		is_speaker_ = root["speaker"].asBool();
		//nim_comp::SessionManager::GetInstance()->SetSpeakerStatus(is_speaker_);
	}
	else
	{
