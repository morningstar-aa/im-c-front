...
public:
	...
	*/
	...
	void OnlyRegShortcuts(std::string screenshots, std::string popMsg);

	void starClip();
	//void ThreadProcessVideo(LPVOID Para);
	void DoClip();
...
private:
	...
	* @return bool true 继续传递控件消息，false 停止传递控件消息
	*/
	bool ShortcutsSettingMenuItemClick(ui::EventArgs* param);
	...
	* @return bool true 继续传递控件消息，false 停止传递控件消息
	*/
	bool GroupSendMenuItemClick(ui::EventArgs* param);
	...
	* @return bool true 继续传递控件消息，false 停止传递控件消息
	*/
	bool BookmarksMenuItemClick(ui::EventArgs* param);
	...
	* @return bool true 继续传递控件消息，false 停止传递控件消息
	*/
	bool SpeakerSettingMenuItemClick(ui::EventArgs* param);
...
public:
	...
	*/
	...
	static bool ParsCustommsg(nim::SysMessage& msg);
...
public:
	...
	* 从服务器同步收藏的图片
	*/
	void GetCustomImgAsyn();

	/**
	* 从本地服务器获取收藏的图片id，进行对比
	*/
	void GetCustomImgFromDB(const std::list<CustomImgInfo>& imgInfo);

	/**
	* 从服务器同步添加的表情
	*/
	void GetCustomEmoticonAsyn();
	...
	*/
	...
	/**
	* 从本地服务器获取添加的表情id，进行对比
	*/
	void GetCustomEmoticonFromDB(const std::list<CustomImgInfo>& imgInfo);
