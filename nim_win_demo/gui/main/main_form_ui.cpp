...
void RegisterShortcuts(LPVOID Para)
{
	...
	shortcutMsgBox *registerMsgBox = shortcutMsgBox::getInstence();

	/*MainForm * main_form = (MainForm *)(nim_ui::WindowsManager::GetInstance()->GetWindow(MainForm::kClassName, MainForm::kClassName));

	OnShortcutsCallBack cb_show_main = [main_form]{
	main_form->ShowWindow();
	};*/
...
void MainForm::OnlyRegShortcuts(std::string screenshots, std::string popMsg)
{
	shortcutMsgBox *registerMsgBox = shortcutMsgBox::getInstence();
	registerMsgBox->reRegisterShortcuts(screenshots, popMsg);
}
void MainForm::RegShortcuts()
{
	_beginthread(RegisterShortcuts, 0, this);
	return;
	//shortcutMsgBox::getInstence()->unRegisterShortcuts();
}

void BeginAsynCustomImg()
{
	MainForm * main_form = (MainForm *)(nim_ui::WindowsManager::GetInstance()->GetWindow(MainForm::kClassName, MainForm::kClassName));
	main_form->GetCustomEmoticonAsyn();
	//main_form->SetTaskbarTitle(nbase::UTF8ToUTF16(nim_comp::LoginManager::GetInstance()->GetLoginName()));
}
...
bool MainForm::ShortcutsSettingMenuItemClick(ui::EventArgs* param)
{
	nim_ui::WindowsManager::GetInstance()->ShowShortcutsSettingForm();
	return true;
}

bool MainForm::GroupSendMenuItemClick(ui::EventArgs* param)
{
	nim_ui::WindowsManager::GetInstance()->ShowGroupSendForm();
	return true;
}

bool MainForm::BookmarksMenuItemClick(ui::EventArgs* param)
{
	nim_ui::WindowsManager::GetInstance()->ShowBookmarksForm();
	return true;
}

bool MainForm::SpeakerSettingMenuItemClick(ui::EventArgs* param)
{
