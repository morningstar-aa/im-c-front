...
void AboutForm::InitWindow()
{
	...
	{
		int ver = 0;
		std::string appVersion = app_sdk::AppSDKInterface::GetAppVersion();
		std::wstring wAppVersion = wstring(appVersion.begin(), appVersion.end());
		...
		std::wstring format = multilan->GetStringViaID(L"STRID_ABOUTFORM_VERSION_EX");
		version->SetText(nbase::StringPrintf(format.c_str(), wAppVersion.c_str()));
	}
}
