...
void LoginForm::DoInitUiKit(nim_ui::InitManager::InitMode mode)
{
	...
	bool enable_subscribe_event = true;
	nim_ui::InitManager::GetInstance()->InitUiKit(enable_subscribe_event, mode);
}
void LoginForm::DoBeforeLogin()
{
	std::string username = user_name_edit_->GetUTF8Text();
	nim_comp::LoginManager::GetInstance()->SetLoginName(username);
	...
	nim_ui::UserManager::GetInstance()->InvokeLoginAccount(username, password, ToWeakCallback([this](int code, const std::string& err_msg, const UserInfo &info) {
		if (code == 0)
		{
			info_ = info;
			nim_ui::UserManager::GetInstance()->SetAccountToken(info);
			nim_ui::LoginManager::GetInstance()->SetAccountToken(info.token);

			PublicDB::GetInstance()->CheckGroup();
			//nim_ui::LoginManager::GetInstance()->SetAccountToken(info.token);
			//GetUserDataPath();
			std::wstring user_app_data_dir = QPath::GetUserAppDataDir(info.uid);
			if (!nbase::FilePathIsExist(user_app_data_dir, true))
			{
				nim_ui::UserManager::GetInstance()->InvokeSendWelcome(info.uid, ToWeakCallback([this](int code, const std::string &msg) {

				}));
				//nim_ui::SessionManager::GetInstance()->OpenSessionBox(nim_ui::LoginManager::GetInstance()->GetAccount(), nim::kNIMSessionTypeP2P);
				nim_ui::LoginManager::GetInstance()->SetIsFristRun(true);
			}
			StartLogin(info.uid, info.nimtoken);
		}
		else
		{
...
void LoginForm::DoRegisterAccount()
{
	...
	if (username.empty())
	{
		ShowLoginTip(mls->GetStringViaID(L"STRID_LOGIN_FORM_TIP_ACCOUNT_EMPTY"));
	}
	...
	//	password = QString::GetMd5(password);
		...
		nim_ui::UserManager::GetInstance()->InvokeRegisterAccountEx(username, password, nickname, "", task);
	}
}
...
void LoginForm::InitLoginData()
{
	...
	UTF8String user_header = manager->GetLoginData()->user_header_;//manager->GetUserHeaderPath(manager->GetLoginData()->user_id_);
	std::wstring path = nbase::UTF8ToUTF16(user_header);
...
void LoginForm::OnLoginOK()
{
	...
	PublicDB::GetInstance()->GetLoginData()->user_id_ = nim_ui::LoginManager::GetInstance()->GetAccount();
	...
	nim_comp::UserService::GetInstance()->InvokeUpdateMyIp(nullptr);
}
...
void LoginForm::IsCanRegister()
{
	std::string os = "pc";
	auto task = ToWeakCallback([this](int res, const std::string& err_msg) {
		((ui::Button*)FindControl(L"register_account"))->SetEnabled(res == 0);
	});
	nim_ui::UserManager::GetInstance()->InvokeGetRegisterCan(os, task);
}
void LoginForm::OnUpdateVerMsgBoxCallback(MsgBoxRet ret)
{
	if (ret == MB_YES)
	{
		std::wstring url = urlVersion;// nbase::UTF8ToUTF16(info.url);
	...
	if (shared::LoadXmlFromFile(document, app_ver_path))
	{
		...
		if (root)
		{
			...
			if (child)
			{
				...
				vf = vf.substr(1, vf.length() - 1);
				version = vf + "." + ver;
			}
		}
	}
	nim_ui::UserManager::GetInstance()->InvokeVersionUpdate(os, version, ToWeakCallback([this](int code, const std::string msg, const VersionInfo &info) {
		if (code == 0)
		{
			if (info.updateType == "0")
			{
				urlVersion = nbase::UTF8ToUTF16(info.url);
				//std::wstring app_crash = QCommand::Get(kCmdAppCrash);
				std::wstring content = ui::MutiLanSupport::GetInstance()->GetStringViaID(L"STRID_VERSION_UPDATA");
				MsgboxCallback cb = nbase::Bind(&LoginForm::OnUpdateVerMsgBoxCallback, this, std::placeholders::_1);
				ShowMsgBox(NULL, cb, content, false, L"STRING_TIPS", true, L"STRID_VERSION_DOWNLOAD_VER", true, L"STRING_NO", true);

			}
			else if (info.updateType == "1")
			{
				ShowMsgBox(NULL, MsgboxCallback(), L"STRID_VERSION_UPDATA", true);
				std::wstring url = nbase::UTF8ToUTF16(info.url);
				ShellExecute(NULL, _T("open"), url.data(), NULL, NULL, SW_SHOWNORMAL);
				//thread_->Stop();
				this->Close();
			}
